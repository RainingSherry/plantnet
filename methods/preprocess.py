...
def prepare_data_for_model(file_path, size_factors=True, filter_min_counts=True, logtrans_input=True, normalize_input=True):
    ...
    if label_col is None:
        raise KeyError(f"No cell type label column found. Available obs columns: {list(data.obs.columns)}")
    Y = data.obs[label_col]  # 获取细胞类型标签（ground truth）
    # size_factors 仅在 size_factors=True 时才存在于 obs 中
    if 'size_factors' in data.obs.columns:
        sf = data.obs['size_factors']
    else:
        sf = None
