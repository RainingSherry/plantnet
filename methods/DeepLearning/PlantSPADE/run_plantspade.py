...
from .data import load_sc_dataset, make_dataloader
from .eval import cluster_and_evaluate
from .train import ScSpade, extract_embeddings, initialize_cluster_centers, train_epoch
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        # 定期在验证集上评估，用KMeans聚类计算NMI，选择最佳直接嵌入
    ...
    print("=" * 70)
    # 提取两种嵌入：(1) 直接编码器输出 (2) 经DDPM逆向过程精炼后的嵌入
    extracted = extract_embeddings(
        model,
        eval_loader,
        device,
    ...
    # 在两种嵌入上分别做KMeans聚类，评估NMI/ARI等指标
