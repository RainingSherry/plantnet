...
class Model(nn.Module):
    ...
    def cross_view_consistency(self, Z1, Z2, adj):
        n_samples = Z1.size()[0]
        chunk_size = int(getattr(self, 'loss_chunk_size', 0) or 0)
        if chunk_size > 0 and n_samples > chunk_size:
            z1 = F.normalize(Z1, dim=1, p=2)
            z2 = F.normalize(Z2, dim=1, p=2)
            total = Z1.new_tensor(0.0)
            for start in range(0, n_samples, chunk_size):
                end = min(start + chunk_size, n_samples)
                S = torch.mm(z1[start:end], z2.t())
                target = adj[start:end].to(device=self.device, dtype=S.dtype).clone()
                local = torch.arange(end - start, device=self.device)
                target[local, start + local] += 1.0
                total = total + torch.sum((S - target) ** 2)
            self.sim_matrix_z = None
    ...
    def neighborhood_contrastive_alignment(self, x_1, x_2, adj, t=1.0):
        ...
        chunk_size = int(getattr(self, 'loss_chunk_size', 0) or 0)
        if chunk_size > 0 and n_samples > chunk_size:
            p_1 = F.softmax(x_1 / t, dim=-1)
            p_2 = F.softmax(x_2 / t, dim=-1)
            log_p_1 = torch.log(p_1)
            log_p_2 = torch.log(p_2)
            entropy_1 = torch.sum(p_1 * log_p_1, dim=-1)
            entropy_2 = torch.sum(p_2 * log_p_2, dim=-1)
            row_losses = []
            for start in range(0, n_samples, chunk_size):
                end = min(start + chunk_size, n_samples)
                kl_12 = entropy_1[start:end].view(-1, 1) - torch.matmul(p_1[start:end], log_p_2.t())
                kl_21_t = entropy_2.view(1, -1) - torch.matmul(log_p_1[start:end], p_2.t())
                dist_matrix = kl_12 + kl_21_t
                adj_chunk = adj[start:end].to(device=self.device, dtype=dist_matrix.dtype)
                mask = adj_chunk.clone()
                local = torch.arange(end - start, device=self.device)
                mask[local, start + local] += 1.0
                sum_dist_neighbors = torch.sum(dist_matrix * mask, dim=-1) / (torch.sum(adj_chunk, dim=-1) + 1)
                diag_dist = dist_matrix[local, start + local]
                sum_sample_dist = (torch.sum(dist_matrix, dim=-1) - diag_dist) / (n_samples - 1)
                row_losses.append(sum_dist_neighbors / sum_sample_dist)
            return torch.mean(torch.cat(row_losses))
