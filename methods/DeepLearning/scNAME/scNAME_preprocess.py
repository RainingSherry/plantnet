...
def normalize(adata, copy=True, highly_genes = None, filter_min_counts=True, size_factors=True, normalize_input=True, logtrans_input=True):
    ...
    # Clean inf/nan early — prevents downstream pd.cut failures in highly_variable_genes
    ...
    if highly_genes != None:
        ...
        # Cleanup inf/nan AFTER subset (subset=True rebuilds the sparse matrix and can introduce NaN)
        X = adata.X
        if sp.sparse.issparse(X):
            X_data = X.data.copy()
            X_data = np.nan_to_num(X_data, nan=0.0, posinf=0.0, neginf=0.0)
            adata.X = sp.sparse.csr_matrix((X_data, X.indices, X.indptr), shape=X.shape)
        else:
            adata.X = np.nan_to_num(np.asarray(X), nan=0.0, posinf=0.0, neginf=0.0)
    if normalize_input:
        sc.pp.scale(adata)
    return adata
