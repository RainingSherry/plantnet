...
from loss import ConditionalMultiMaskLoss
from model import ConditionalMultiMaskScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank54_conditional_multi_mask_full"
DISPLAY_NAME = "scMAE + Conditional-MAE multi-mask"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent Conditional-MAE multi-mask scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--input_mask_prob", type=float, default=0.35)
    p.add_argument("--latent_mask_prob", type=float, default=0.35)
    p.add_argument("--replace_prob", type=float, default=0.25)
    p.add_argument("--masked_data_weight", type=float, default=0.75)
    p.add_argument("--mask_loss_weight", type=float, default=0.7)
    p.add_argument("--latent_mask_weight", type=float, default=0.05)
    p.add_argument("--condition_weight", type=float, default=0.02)
...
@torch.no_grad()
def extract_embedding(model: ConditionalMultiMaskScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, mask_scores = [], [], []
    for _, x_cpu, _, y in loader:
        x = x_cpu.to(device)
        out = model(x, latent_mask_ratio=0.0)
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, input_mask_rate: float, latent_mask_rate: float) -> dict:
    if preds is None:
    ...
    return {
        "edge_survival": float(1.0 - input_mask_rate),
        ...
        "input_mask_rate": float(input_mask_rate),
        "latent_mask_rate": float(latent_mask_rate),
        "diagnostic_note": "No NeighborMix is used. Conditional-MAE is adapted as input masking followed by representation-stage latent masking.",
    }
...
def main() -> int:
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "rank54_core": "Conditional-MAE two-shot masking: expression input mask and latent-stage mask"}, str(save_dir / "preprocess_config.json"))
    dataset = ExpressionDataset(encoder_data, log_expr, labels)
    ...
    model = ConditionalMultiMaskScMAE(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, args.latent_dim, args.dropout).to(device)
    criterion = ConditionalMultiMaskLoss(args.masked_data_weight, args.mask_loss_weight, args.latent_mask_weight, args.condition_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "latent_mask_loss", "condition_loss", "variance_loss", "effective_mask_rate", "latent_mask_rate"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            corrupted, mask = corrupt(x, args.input_mask_prob, args.replace_prob)
            with torch.no_grad():
                clean_latent = model.encoder_stage1(x)
            out = model(corrupted, args.latent_mask_prob)
            loss, parts, _ = criterion(out, y, mask, clean_latent)
            ...
            sums["latent_mask_rate"] += float(out["latent_mask"].mean().detach().cpu())
            nb += 1
        for k in keys:
            history[k].append(sums[k] / max(1, nb))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} input_mask={history['effective_mask_rate'][-1]:.3f} latent_mask={history['latent_mask_rate'][-1]:.3f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + conditional multi-mask"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], history["latent_mask_rate"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_conditional_multi_mask_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_conditional_multi_mask_scmae.h5ad", compression="gzip")
