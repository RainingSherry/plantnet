...
from loss import apply_feature_mask, bgrl_loss, build_knn_adjacency, drop_edges
from model import BGRLGraphBootstrapScMAE


def parse_args():
    parser = argparse.ArgumentParser("rank26 BGRL graph bootstrap scMAE")
    ...
    parser.add_argument("--feature_mask_ratio", type=float, default=0.25)
    parser.add_argument("--column_drop_prob", type=float, default=0.05)
...
@torch.no_grad()
def extract_features(model: BGRLGraphBootstrapScMAE, x: np.ndarray, device: torch.device, batch_size: int, graph_top_k: int) -> np.ndarray:
    model.eval()
    outputs = []
...
def main():
    ...
    preprocess_config["bgrl_graph"] = "mini-batch KNN graph with stochastic edge dropout in two views"
    preprocess_config["mask_semantics"] = "1 = expression feature set to zero by feature/node-view corruption"
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, xb_cpu, _ in loader:
            ...
            view1, mask1 = apply_feature_mask(clean, args.feature_mask_ratio, args.column_drop_prob)
            view2, mask2 = apply_feature_mask(clean, args.feature_mask_ratio, args.column_drop_prob)
            ...
            sums["bootstrap_loss"] += float(parts.bootstrap.detach().cpu())
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(
                f"rank26 epoch={epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} "
                f"bootstrap={history['bootstrap_loss'][-1]:.4f} ema={momentum:.5f}",
                flush=True,
            )
    ...
    if not args.skip_eval:
        result = family.write_kmeans_known_k_outputs(
            ...
            n_clusters,
            {
                "rank": 26,
                "source_paper": "BGRL",
                "mask_semantics": "1 = expression feature set to zero by BGRL view corruption",
            },
        )
