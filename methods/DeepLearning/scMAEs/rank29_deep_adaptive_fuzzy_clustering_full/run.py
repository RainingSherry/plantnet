...
from loss import DeepAdaptiveFuzzyLoss
from model import DeepAdaptiveFuzzyScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank29_deep_adaptive_fuzzy_clustering_full"
DISPLAY_NAME = "scMAE + deep adaptive fuzzy clustering"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent deep adaptive fuzzy clustering scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--fuzzy_weight", type=float, default=0.08)
...
@torch.no_grad()
def initialize_centers(model: DeepAdaptiveFuzzyScMAE, anchor: np.ndarray, n_clusters: int, seed: int, device: torch.device) -> None:
...
@torch.no_grad()
def extract_embedding(model: DeepAdaptiveFuzzyScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, conf = [], [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, confidence: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None) -> dict:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix is used. Adaptive fuzzy membership strengthens only high-confidence core cells and leaves boundary cells entropy-tolerant.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "fuzzy_target": "core-cell KL(P||Q) after warmup, boundary entropy tolerance, balanced fuzzy memberships", "effective_confidence_threshold": effective_confidence_threshold, "svd_anchor": f"TruncatedSVD({args.anchor_dim}) on scaled encoder input"}, str(save_dir / "preprocess_config.json"))
    ...
    model = DeepAdaptiveFuzzyScMAE(encoder_data.shape[1], n_clusters, args.hidden_size, args.anchor_dim, args.dropout).to(device)
    initialize_centers(model, anchor, n_clusters, args.seed, device)
    criterion = DeepAdaptiveFuzzyLoss(args.masked_data_weight, args.mask_weight, args.fuzzy_weight, args.entropy_weight, args.balance_weight, args.separation_weight, args.anchor_weight, args.variance_weight)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {k: [] for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "fuzzy_kl_loss", "boundary_entropy_loss", "balance_loss", "center_separation_loss", "anchor_loss", "variance_loss", "core_fraction", "membership_confidence", "membership_entropy", "effective_mask_rate"]}
    history["stage"] = stage
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, anchor_cpu, _ in train_loader:
            ...
            fuzzy_active = epoch > args.warmup_epochs
            loss, parts = criterion(out, target, mask, anchor_batch, model.cluster_centers, fuzzy_active, effective_confidence_threshold)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} fuzzy={history['fuzzy_kl_loss'][-1]:.4f} core={history['core_fraction'][-1]:.4f} conf={history['membership_confidence'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + adaptive fuzzy clustering"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_deep_adaptive_fuzzy_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_deep_adaptive_fuzzy_scmae.h5ad", compression="gzip")
