...
from loss import AnomalyAssociationBoundaryLoss
from model import AnomalyAssociationScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank25_anomaly_association_boundary_full"
DISPLAY_NAME = "scMAE + anomaly association boundary"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent Anomaly-Association boundary scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--assoc_weight", type=float, default=0.04)
    parser.add_argument("--latent_consistency_weight", type=float, default=0.03)
    parser.add_argument("--boundary_weight", type=float, default=0.02)
    parser.add_argument("--variance_weight", type=float, default=0.01)
    return parser.parse_args()


def coexpression_order(log_expr: np.ndarray, seed: int, max_cells: int = 10000) -> np.ndarray:
    rng = np.random.default_rng(seed)
    x = np.asarray(log_expr, dtype=np.float32)
    if x.shape[0] > max_cells:
        x = x[rng.choice(x.shape[0], size=max_cells, replace=False)]
    x = x - x.mean(axis=0, keepdims=True)
    denom = np.maximum(x.std(axis=0, keepdims=True), 1e-6)
    x = x / denom
    cov = np.matmul(x.T, x) / max(1, x.shape[0] - 1)
    cov = np.nan_to_num(cov, nan=0.0, posinf=0.0, neginf=0.0)
    values, vectors = np.linalg.eigh(cov)
    loading = vectors[:, int(np.argmax(values))]
    return np.argsort(loading).astype(np.int64)


@torch.no_grad()
def extract_embedding(model: AnomalyAssociationScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, risk = [], [], []
    for _, x, _, y in loader:
        out = model(x.to(device))
        emb.append(out["latent"].detach().cpu().numpy())
        risk.append(out["association_risk"].detach().cpu().numpy())
        labels.append(y.numpy())
    return (
        np.nan_to_num(np.concatenate(emb).astype(np.float32)),
        np.concatenate(labels).astype(np.int64),
        np.nan_to_num(np.concatenate(risk).astype(np.float32)),
    )
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, risk: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None) -> dict:
    if preds is None:
    ...
    if finite_risk.size:
        threshold = float(np.quantile(finite_risk, 0.90))
        risk_fraction = float(np.mean(finite_risk >= threshold))
        centered = finite_risk - finite_risk.min()
        hist = np.histogram(centered, bins=10)[0].astype(np.float64)
        hist = hist / max(1.0, hist.sum())
        hist = hist[hist > 0]
        assoc_entropy = float(-(hist * np.log(hist)).sum() / np.log(10.0))
        risk_mean = float(finite_risk.mean())
        risk_std = float(finite_risk.std())
    else:
        threshold = risk_fraction = assoc_entropy = risk_mean = risk_std = float("nan")
    collapse = bool((not np.isfinite(var)) or var < 1e-8 or mass_min < 0.001 or mass_max > 0.95)
    return {
        "edge_survival": 1.0,
        ...
        "rare_risk_fraction": risk_fraction,
        "embedding_variance": var,
        "cluster_mass_min": mass_min,
        "cluster_mass_max": mass_max,
        "collapse_warning": collapse,
        "association_risk_mean": risk_mean,
        "association_risk_std": risk_std,
        "association_risk_p90": threshold,
        "association_boundary_entropy": assoc_entropy,
        "diagnostic_note": "No NeighborMix is used. Association discrepancy is adapted from time-series anomaly attention to ordered gene-module boundary diagnostics.",
    }
...
def main() -> int:
    ...
    order = coexpression_order(log_expr, args.seed)
    encoder_data = encoder_data[:, order]
    log_expr = log_expr[:, order]
    gene_names = target_bundle.gene_names.astype(str)[order]
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "gene_order": "covariance_first_eigenvector_on_log_expr", "association_target": "masked-view association discrepancy should match clean-view discrepancy"}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_names.npy", gene_names)
    np.save(save_dir / "gene_order.npy", order.astype(np.int64))
    ...
    model = AnomalyAssociationScMAE(encoder_data.shape[1], args.patch_size, args.hidden_size, args.n_heads, args.depth, args.dropout).to(device)
    criterion = AnomalyAssociationBoundaryLoss(args.masked_data_weight, args.mask_weight, args.assoc_weight, args.latent_consistency_weight, args.boundary_weight, args.variance_weight)
    ...
    history = {
        ...
        "reconstruction_loss": [],
        "mask_loss": [],
        "association_stability_loss": [],
        "latent_consistency_loss": [],
        "boundary_loss": [],
        "variance_loss": [],
        "association_risk_mean": [],
        "association_risk_std": [],
        "effective_mask_rate": [],
        "stage": stage,
    }
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in history if isinstance(history[k], list) and k != "effective_mask_rate"}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            out = model(corrupted)
            clean_out = model(x)
            loss, parts = criterion(out, clean_out, target, mask)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} assoc={history['association_stability_loss'][-1]:.4f} boundary={history['boundary_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")

    embedding, labels_out, association_risk = extract_embedding(model, full_loader, device)
    ...
    np.save(save_dir / "association_risk.npy", association_risk.astype(np.float32))
    family.save_embedding_h5(save_dir / "embedding.h5", embedding, labels_out)
    save_json(history, str(save_dir / "training_history.json"))
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args), "gene_names": gene_names}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + covariance ordered gene modules"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, association_risk, n_clusters, args.seed, preds)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_anomaly_association_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_anomaly_association_scmae.h5ad", compression="gzip")
