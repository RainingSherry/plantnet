...
from loss import make_patch_mask, patch_mask_to_gene_mask, scmamba_masked_loss
from model import ScMambaSSMScMAE


def parse_args():
    parser = argparse.ArgumentParser("rank11 scMamba selective-SSM scMAE")
...
@torch.no_grad()
def extract_features(model: ScMambaSSMScMAE, x: np.ndarray, device: torch.device, batch_size: int) -> np.ndarray:
    model.eval()
    outputs = []
...
def main():
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, xb_cpu, _ in loader:
            ...
            sums["token_smoothness_loss"] += float(parts.token_smoothness.detach().cpu())
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(
                f"rank11 epoch={epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} "
                f"recon={history['reconstruction_loss'][-1]:.4f} mask={history['mask_loss'][-1]:.4f}",
                flush=True,
            )
    ...
    if not args.skip_eval:
        result = family.write_kmeans_known_k_outputs(
            ...
            {
                "rank": 11,
                "source_paper": "scMamba",
                "mask_semantics": "1 = gene belongs to a masked patch replaced by the learned SSM mask token",
            },
        )
