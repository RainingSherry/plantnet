...
from loss import ScVAEDerLoss
from model import ScVAEDerScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank57_scvaeder_latent_diffusion_full"
DISPLAY_NAME = "scMAE + scVAEDer latent diffusion"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent scVAEDer latent-diffusion scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--diffusion_weight", type=float, default=0.02)
...
@torch.no_grad()
def extract_embedding(model: ScVAEDerScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def main() -> int:
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "raw_count_branch": raw_note, "rank57_core": "scVAEDer latent diffusion denoising plus optional raw-count ZINB likelihood"}, str(save_dir / "preprocess_config.json"))

    dataset = ScVAEDerDataset(encoder_data, log_expr, labels, raw_counts, size_factor)
    ...
    model = ScVAEDerScMAE(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, args.dropout).to(device)
    criterion = ScVAEDerLoss(args.masked_data_weight, args.mask_loss_weight, args.diffusion_weight, args.kl_weight, args.zinb_weight if raw_available else 0.0, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "diffusion_loss", "kl_loss", "zinb_loss", "variance_loss", "effective_mask_rate"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} diffusion={history['diffusion_loss'][-1]:.4f} zinb={history['zinb_loss'][-1]:.4f} mask={history['effective_mask_rate'][-1]:.3f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + optional aligned raw-count ZINB + latent diffusion denoising"})
    ...
    if not args.no_save_h5ad:
        enc.adata.obsm["X_scvaeder_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_scvaeder_scmae.h5ad", compression="gzip")
