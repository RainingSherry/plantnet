...
from model import FCMMaskUnmaskScMAE
from loss import FCMMaskUnmaskLoss


METHOD_DIR = "suture04_fcm_mask_unmask_fusion_full"
METHOD_NAME = "scMAE + FCM masked/unmasked latent correction"
BASELINES = {
...
def parse_args():
    ...
    p.add_argument("--module_weight", type=float, default=0.35)
    p.add_argument("--min_gate", type=float, default=0.05)
    p.add_argument("--target_gate", type=float, default=0.12)
    p.add_argument("--gate_budget_weight", type=float, default=0.04)
    return p.parse_args()
...
def collect_gate(model, loader, device):
    model.eval()
    gates = []
    with torch.no_grad():
        for _, x, _ in loader:
            out = model(x.to(device))
            gates.append(out["correction_gate"].detach().cpu().numpy())
...
def diagnostics(embedding, labels, pred, gate):
    ...
    return {
        ...
        "rare_risk_fraction": float((gate.mean(axis=1) > 0.65).mean()),
        ...
        "correction_gate_mean": float(gate.mean()),
        "correction_gate_std": float(gate.std()),
        "diagnostic_note": "FCM is adapted to masked/unmasked latent correction; clean view is a detached teacher context and no cell mixing is used.",
    }
...
def main():
    ...
    loss_fn = FCMMaskUnmaskLoss(gate_budget_weight=args.gate_budget_weight, target_gate=args.target_gate)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={rec['total']:.4f} rec={rec['reconstruction']:.4f} mask={rec['mask_bce']:.4f} gate={rec['gate_mean']:.3f}")
    ...
    gate = collect_gate(model, eval_loader, device)
    np.save(out / "correction_gate.npy", gate)
    fixed, pred = {}, None
