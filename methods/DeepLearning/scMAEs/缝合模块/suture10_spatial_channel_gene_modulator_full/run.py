...
from model import SpatialChannelGeneModulatorScMAE
from loss import SpatialChannelGeneModulatorLoss


METHOD_DIR = "suture10_spatial_channel_gene_modulator_full"
METHOD_NAME = "scMAE + spatial-channel gene modulator"
BASELINES = {
...
def parse_args():
    ...
    p.add_argument("--modulator_weight", type=float, default=0.05)
    p.add_argument("--target_cell_gate", type=float, default=0.12)
    return p.parse_args()
...
@torch.no_grad()
def collect_gates(model, loader, device):
    model.eval()
    channel, cell = [], []
    for _, x, _ in loader:
        out = model.encode_modulated(x.to(device))
        channel.append(out["channel_gate"].detach().cpu().numpy())
        cell.append(out["cell_gate"].detach().cpu().numpy())
    return np.concatenate(channel, axis=0).astype(np.float32), np.concatenate(cell, axis=0).astype(np.float32)


def diagnostics(embedding, labels, pred, channel_gate, cell_gate):
    if pred is None:
    ...
    return {
        ...
        "cell_gate_mean": float(cell_gate.mean()),
        "cell_gate_std": float(cell_gate.std()),
        "channel_gate_mean": float(channel_gate.mean()),
        "channel_gate_std": float(channel_gate.std()),
        "diagnostic_note": "SCFM rewritten as latent channel/cell modulation; no image reshape, no graph, and no cell mixing.",
    }
...
def main():
    ...
    model = SpatialChannelGeneModulatorScMAE(
        enc.data.shape[1],
        args.hidden_dim,
        args.latent_dim,
        args.dropout,
        args.mask_prob,
        args.modulator_weight,
    ).to(device)
    loss_fn = SpatialChannelGeneModulatorLoss(target_cell_gate=args.target_cell_gate)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={rec['total']:.4f} rec={rec['reconstruction']:.4f} mask={rec['mask_bce']:.4f} cell={rec['cell_gate_mean']:.3f} chan={rec['channel_gate_mean']:.3f}")
    ...
    channel_gate, cell_gate = collect_gates(model, eval_loader, device)
    np.save(out / "channel_gate.npy", channel_gate)
    np.save(out / "cell_gate.npy", cell_gate)
    fixed, pred = {}, None
    ...
    diag = diagnostics(embedding, labels, pred, channel_gate, cell_gate)
