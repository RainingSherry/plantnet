...
from model import LaSEALatentSemanticScMAE
from loss import LaSEALatentSemanticLoss


METHOD_DIR = "suture11_lasea_latent_semantic_aggregation_full"
METHOD_NAME = "scMAE + LaSEA latent semantic aggregation"
BASELINES = {
...
def parse_args():
    ...
    p.add_argument("--semantic_weight", type=float, default=0.05)
    p.add_argument("--semantic_loss_weight", type=float, default=0.035)
    p.add_argument("--target_gate", type=float, default=0.10)
    return p.parse_args()
...
@torch.no_grad()
def collect_semantic(model, loader, device):
    model.eval()
    gates, attn = [], []
    for _, x, _ in loader:
        out = model.encode_semantic(x.to(device))
        gates.append(out["semantic_gate"].detach().cpu().numpy())
        attn.append(out["semantic_attention"].detach().cpu().numpy())
    return np.concatenate(gates, axis=0).astype(np.float32), np.concatenate(attn, axis=0).astype(np.float32)


def diagnostics(embedding, labels, pred, gate, attention):
    if pred is None:
    ...
    return {
        ...
        "semantic_gate_mean": float(gate.mean()),
        "semantic_gate_std": float(gate.std()),
        "semantic_attention_mean": float(attention.mean()),
        "semantic_attention_std": float(attention.std()),
        "diagnostic_note": "LaSEA is rewritten as latent 1D multi-dilation semantic aggregation with detached clean-view semantic consistency.",
    }
...
def main():
    ...
    model = LaSEALatentSemanticScMAE(enc.data.shape[1], args.hidden_dim, args.latent_dim, args.dropout, args.mask_prob, args.semantic_weight).to(device)
    loss_fn = LaSEALatentSemanticLoss(semantic_weight=args.semantic_loss_weight, target_gate=args.target_gate)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={rec['total']:.4f} rec={rec['reconstruction']:.4f} mask={rec['mask_bce']:.4f} sem={rec['semantic_loss']:.4f} gate={rec['semantic_gate_mean']:.3f}")
    ...
    gate, attention = collect_semantic(model, eval_loader, device)
    np.save(out / "semantic_gate.npy", gate)
    np.save(out / "semantic_attention.npy", attention)
    fixed, pred = {}, None
    ...
    diag = diagnostics(embedding, labels, pred, gate, attention)
