...
class LaSEALatentSemanticLoss(nn.Module):
    """scMAE objective with clean-view latent semantic aggregation consistency."""

    def __init__(
        self,
        reconstruction_weight: float = 1.0,
        mask_weight: float = 0.2,
        semantic_weight: float = 0.035,
    ...
    def forward(self, outputs: dict, target: torch.Tensor) -> tuple[torch.Tensor, dict]:
        ...
        semantic_loss = F.smooth_l1_loss(outputs["semantic_summary"], outputs["clean_semantic_summary"].detach())
        delta_penalty = outputs["semantic_delta"].pow(2).mean()
        gate_budget = (outputs["semantic_gate"].mean() - self.target_gate).pow(2)
        total = (
        ...
        parts = {
            ...
            "semantic_gate_mean": float(outputs["semantic_gate"].mean().detach().cpu()),
            "attention_mean": float(outputs["semantic_attention"].mean().detach().cpu()),
