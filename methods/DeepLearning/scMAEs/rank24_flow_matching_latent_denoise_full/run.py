...
from loss import FlowMatchingLatentLoss
from model import FlowMatchingLatentScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank24_flow_matching_latent_denoise_full"
DISPLAY_NAME = "scMAE + latent flow matching denoise"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent latent Flow Matching scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--flow_weight", type=float, default=0.08)
...
@torch.no_grad()
def extract_embedding(model: FlowMatchingLatentScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, flow_target_norm: float, flow_pred_norm: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "flow_target_norm": float(flow_target_norm),
        "flow_pred_norm": float(flow_pred_norm),
        "diagnostic_note": "No NeighborMix graph is used; latent Flow Matching is used only as denoising regularization, not for generated sample evaluation.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "flow_target": "latent OT-style conditional flow matching from Gaussian noise to clean scMAE latent"}, str(save_dir / "preprocess_config.json"))
    ...
    model = FlowMatchingLatentScMAE(encoder_data.shape[1], args.hidden_size, args.time_dim, args.dropout).to(device)
    criterion = FlowMatchingLatentLoss(args.masked_data_weight, args.mask_weight, args.flow_weight, args.denoise_weight, args.variance_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "flow_loss": [], "denoise_loss": [], "variance_loss": [], "flow_target_norm": [], "flow_pred_norm": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "flow_loss", "denoise_loss", "variance_loss", "flow_target_norm", "flow_pred_norm"]}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            t = torch.rand(z_clean.shape[0], device=device).clamp(0.02, 0.98)
            t_pad = t[:, None]
            sigma_t = 1.0 - (1.0 - args.sigma_min) * t_pad
            z_t = t_pad * z_clean.detach() + sigma_t * x0
            flow_target = (z_clean.detach() - (1.0 - args.sigma_min) * z_t) / sigma_t.clamp_min(1e-4)
            flow_pred = model.predict_flow(z_t, t)
            z1_hat = z_t + (1.0 - t_pad) * flow_pred
            loss, parts = criterion(out, target, mask, flow_pred, flow_target, z1_hat, z_clean)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} flow={history['flow_loss'][-1]:.4f} denoise={history['denoise_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + latent flow matching regularizer"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, history["flow_target_norm"][-1], history["flow_pred_norm"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_flow_matching_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_flow_matching_scmae.h5ad", compression="gzip")
