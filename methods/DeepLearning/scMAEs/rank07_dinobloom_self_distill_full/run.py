...
from loss import DinoBloomDistillLoss
from model import DinoBloomScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank07_dinobloom_self_distill_full"
DISPLAY_NAME = "scMAE + DinoBloom self-distillation"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent DinoBloom self-distilled scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--n_prototypes", type=int, default=256)
...
@torch.no_grad()
def extract_embedding(model: DinoBloomScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, center_norm: float, teacher_entropy: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "collapse_warning": bool((not np.isfinite(var)) or var < 1e-8 or mass_min < 0.001 or mass_max > 0.95 or teacher_entropy < 0.1),
        "center_norm": float(center_norm),
        "teacher_entropy": float(teacher_entropy),
        "diagnostic_note": "No NeighborMix graph is used; EMA teacher and centered prototype distillation are used.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "self_distillation": "EMA teacher + centered prototype head"}, str(save_dir / "preprocess_config.json"))
    ...
    student = DinoBloomScMAE(encoder_data.shape[1], args.hidden_size, args.projection_dim, args.n_prototypes, args.dropout).to(device)
    teacher = copy.deepcopy(student).to(device)
    teacher.eval()
    for param in teacher.parameters():
        param.requires_grad_(False)
    center = torch.zeros(1, args.n_prototypes, device=device)
    criterion = DinoBloomDistillLoss(args.masked_data_weight, args.mask_weight, args.distill_weight, args.student_temp, args.teacher_temp)
    optimizer = torch.optim.Adam(student.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "distill_loss": [], "effective_mask_rate": [], "center_norm": [], "teacher_entropy": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        student.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "distill_loss"]}
        mask_sum = 0.0
        entropy_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            strong, mask = student.corrupt_swap(x, args.mask_prob)
            weak, _ = student.corrupt_swap(x, max(0.05, args.mask_prob * 0.25))
            student_out = student(strong)
            with torch.no_grad():
                teacher_logits = teacher(weak)["proto_logits"]
            loss, parts = criterion(student_out, teacher_logits, center, target, mask)
            ...
            with torch.no_grad():
                ...
                center.mul_(args.center_momentum).add_(batch_center, alpha=1.0 - args.center_momentum)
                probs = torch.softmax((teacher_logits - center) / args.teacher_temp, dim=1)
        ...
        history["center_norm"].append(float(center.norm().detach().cpu()))
        history["teacher_entropy"].append(entropy_sum / max(1, n_batches))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} distill={history['distill_loss'][-1]:.4f} entropy={history['teacher_entropy'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")

    embedding, labels_out = extract_embedding(teacher, test_loader, device)
    ...
    torch.save({"student": student.state_dict(), "teacher": teacher.state_dict(), "optimizer": optimizer.state_dict(), "center": center.detach().cpu(), "args": vars(args), "gene_names": target_bundle.gene_names.astype(str)}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + DINO-style self-distillation"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, history["center_norm"][-1], history["teacher_entropy"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_dinobloom_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_dinobloom_scmae.h5ad", compression="gzip")
