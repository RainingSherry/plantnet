...
from loss import AsymmetricMAELoss
from model import AsymmetricMAEScRNA
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank48_mae_asymmetric_decoder_full"
DISPLAY_NAME = "scMAE + MAE asymmetric decoder"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent MAE asymmetric-decoder scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
@torch.no_grad()
def extract_embedding(model: AsymmetricMAEScRNA, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float) -> dict:
    if preds is None:
...
def main() -> int:
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "mae_core": "scMAE swap corruption + asymmetric latent-to-decoder bottleneck + mask-token conditioned decoder"}, str(save_dir / "preprocess_config.json"))
    ...
    model = AsymmetricMAEScRNA(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, args.dropout).to(device)
    criterion = AsymmetricMAELoss(args.masked_data_weight, args.mask_loss_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "recon_loss", "mask_loss", "variance_loss", "effective_mask_rate"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            corrupted, mask = model.swap_mask_view(x, args.mask_prob)
            out = model(corrupted)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} recon={history['recon_loss'][-1]:.4f} mask_loss={history['mask_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.3f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + MAE asymmetric decoder"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, n_clusters, args.seed, preds, history["effective_mask_rate"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_mae_asymmetric_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_mae_asymmetric_scmae.h5ad", compression="gzip")
