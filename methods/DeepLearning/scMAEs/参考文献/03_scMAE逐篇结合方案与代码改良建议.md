...
```text
Teacher-Consensus Manifold scMAE
或
Boundary-aware Teacher NeighborMix scMAE
```

核心假设：

```text
scMAE 的主要缺口是缺少可靠的 cell-cell manifold modeling。
朴素 NeighborMix 能补这个缺口，但会被 noisy self-feedback 破坏。
因此使用 EMA teacher 和 consensus graph 稳定邻居选择，
再通过 boundary-aware mixing 避免过平滑和稀有细胞吞并。
```
...
```text
L = L_scMAE_rec
  + λ_mask L_replaced_detection
  + λ_graph L_teacher_consensus_graph
  + λ_mix L_confidence_neighbor_mix
  + λ_sem L_module/pathway/rank_target
  + λ_boundary L_boundary_or_rare_cell_protection
  + λ_robust L_Huber/NB/ZINB_optional
```

其中 `L_scMAE_rec` 必须是主损失；其他 loss 是结构化适配器，不应压过 scMAE。
...
1. 不再把所有论文都映射成 `variant_defs.py` 中几个权重。
2. 不再用 1 seed 的 Melanoma_5K 结果判定方法优劣。
3. 不再把 scaled expression 直接用于 ZINB、count token、discrete diffusion。
4. 不再把 prototype/SwAV/pseudo-label 在训练早期强加给全部细胞。
5. 不再让 graph 方法直接替换 scMAE 主干或堆多层 GNN。

### P1：下一轮真正值得实现的方向

1. **Teacher-consensus graph**：EMA teacher embedding + mutual-KNN + edge persistence。
2. **Confidence-aware NeighborMix**：只对高置信、稳定、非边界、非 rare-risk 边做 mix。
3. **Boundary / rare-cell protection**：对核心细胞强约束，对边界细胞弱约束，对稀有细胞降低 mix。
4. **Semantic target**：表达值 + gene module/pathway/rank target。
5. **Robust reconstruction**：Huber 默认，NB/ZINB 作为 raw-count 分支可选。
6. **Mechanism diagnostics**：graph purity、edge survival、mixed-cell fraction、rare-cell recall、boundary entropy。

### P2：正式论文模型的推荐消融

```text
scMAE
scMAE + naive NeighborMix
scMAE + EMA teacher graph
scMAE + consensus graph
scMAE + confidence-aware NeighborMix
scMAE + boundary/rare protection
scMAE + semantic target
完整模型
```

每个消融至少跑：
- Melanoma_5K：3 seeds
- Quake_10x_Spleen：3 seeds
- 一个困难数据集：3 seeds
...
```text
我们发现 scMAE 的关键缺口在于缺少可靠的 cell-cell manifold modeling。
朴素 NeighborMix 证明该缺口有价值，但存在 noisy self-feedback。
因此我们提出 teacher-consensus-guided, boundary-aware NeighborMix scMAE，
在保持 scMAE masked reconstruction 主体的同时，
引入稳定、可解释、适配 scRNA 结构的细胞邻域建模机制。
```
