...
from loss import ConsistencyEMALoss
from model import ConsistencyEMAScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank03_consistency_ema_robust_full"
DISPLAY_NAME = "scMAE + consistency EMA robust denoising"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent consistency-EMA scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--pseudo_huber_c", type=float, default=0.03)
...
@torch.no_grad()
def extract_embedding(model: ConsistencyEMAScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None) -> dict:
    ...
    probs = frac[frac > 0]
    ent = float(-(probs * np.log(probs)).sum() / max(np.log(max(2, n_clusters)), 1e-8))
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix graph is used; EMA teacher is used for latent consistency.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "consistency": "EMA teacher clean view"}, str(save_dir / "preprocess_config.json"))
    ...
    student = ConsistencyEMAScMAE(encoder_data.shape[1], args.hidden_size, args.projection_dim, args.dropout).to(device)
    teacher = copy.deepcopy(student).to(device)
    teacher.eval()
    for param in teacher.parameters():
        param.requires_grad_(False)
    criterion = ConsistencyEMALoss(args.reconstruction_weight, args.mask_weight, args.consistency_weight, args.huber_beta, args.pseudo_huber_c)
    optimizer = torch.optim.AdamW(student.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "reconstruction_loss": [], "mask_loss": [], "consistency_loss": [], "effective_mask_rate": [], "sigma_mean": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        student.train()
        sums = {"loss": 0.0, "reconstruction_loss": 0.0, "mask_loss": 0.0, "consistency_loss": 0.0}
        mask_sum, sigma_sum, n_batches = 0.0, 0.0, 0
        for _, x_enc_cpu, x_log_cpu, _ in train_loader:
            x_enc = x_enc_cpu.to(device)
            x_log = x_log_cpu.to(device)
            noisy, mask, sigma = student.corrupt(x_enc, args.mask_prob, args.sigma_min, args.sigma_max)
            student_out = student(noisy, sigma)
            with torch.no_grad():
                teacher_sigma = torch.zeros(x_enc.shape[0], device=device) + args.sigma_min
                teacher_projection = teacher(x_enc, teacher_sigma)["projection"]
            loss, parts = criterion(student_out, teacher_projection, x_log, mask)
            ...
            sigma_sum += float(sigma.mean().detach().cpu())
            n_batches += 1
        for key in sums:
            history[key].append(sums[key] / max(1, n_batches))
        history["effective_mask_rate"].append(mask_sum / max(1, n_batches))
        history["sigma_mean"].append(sigma_sum / max(1, n_batches))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} rec={history['reconstruction_loss'][-1]:.4f} mask={history['mask_loss'][-1]:.4f} cons={history['consistency_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log_expr target", "ema_decay": float(args.ema_decay)})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_consistency_ema_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_consistency_ema_scmae.h5ad", compression="gzip")
