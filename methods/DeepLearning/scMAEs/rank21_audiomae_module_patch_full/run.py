...
from loss import AudioMAEModulePatchLoss
from model import AudioMAEModulePatchScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank21_audiomae_module_patch_full"
DISPLAY_NAME = "scMAE + AudioMAE module patch"
...
class ExprDataset(Dataset):
    def __init__(self, encoder_data: np.ndarray, log_expr: np.ndarray, module_targets: np.ndarray, labels: np.ndarray):
        ...
        self.module_targets = torch.as_tensor(module_targets, dtype=torch.float32)
    ...
    def __getitem__(self, idx: int):
        return int(idx), self.encoder_data[idx], self.log_expr[idx], self.module_targets[idx], self.labels[idx]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent AudioMAE module-patch scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--mask_ratio", type=float, default=0.62)
    parser.add_argument("--structured_prob", type=float, default=0.35)
    parser.add_argument("--masked_data_weight", type=float, default=0.8)
    parser.add_argument("--mask_weight", type=float, default=0.6)
...
def coexpression_gene_order(log_expr: np.ndarray, n_anchors: int = 12, max_cells: int = 5000, seed: int = 42) -> np.ndarray:
    ...
    x = np.nan_to_num(x)
    var = x.var(axis=0)
    anchors = np.argsort(var)[-min(n_anchors, x.shape[1]) :]
    xz = (x - x.mean(axis=0, keepdims=True)) / (x.std(axis=0, keepdims=True) + 1e-6)
    az = xz[:, anchors]
    corr = (xz.T @ az) / max(1, xz.shape[0] - 1)
    assign = np.argmax(np.abs(corr), axis=1)
    strength = np.max(np.abs(corr), axis=1)
    direction = np.sign(corr[np.arange(corr.shape[0]), assign])
    return np.lexsort((-strength, direction, assign)).astype(np.int64)


def module_spectral_targets(ordered_log: np.ndarray, patch_size: int) -> np.ndarray:
    n_patches = (ordered_log.shape[1] + patch_size - 1) // patch_size
    padded = np.pad(ordered_log, ((0, 0), (0, n_patches * patch_size - ordered_log.shape[1])), mode="constant")
    patches = padded.reshape(ordered_log.shape[0], n_patches, patch_size)
    mean = patches.mean(axis=2)
    std = patches.std(axis=2)
    if patch_size > 1:
        local_energy = np.abs(np.diff(patches, axis=2)).mean(axis=2)
    else:
        local_energy = np.zeros_like(mean)
    return np.stack([mean, std, local_energy], axis=2).astype(np.float32)


@torch.no_grad()
def extract_embedding(model: AudioMAEModulePatchScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, module_var: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "module_target_variance": float(module_var),
        "diagnostic_note": "No NeighborMix graph is used; AudioMAE-style high-ratio module patch masking is applied after coexpression ordering.",
    }
...
def main() -> int:
    ...
    gene_order = coexpression_gene_order(log_expr, seed=args.seed)
    ordered_encoder = np.asarray(encoder_data[:, gene_order], dtype=np.float32)
    ordered_log = np.asarray(log_expr[:, gene_order], dtype=np.float32)
    module_targets = module_spectral_targets(ordered_log, args.patch_size)

    save_json(target_bundle.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "gene_order_source": "coexpression anchor order from log_expr", "module_target_source": "mean/std/local-diff spectral summaries from log_expr patches"}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_order.npy", gene_order.astype(np.int64))
    np.save(save_dir / "gene_names.npy", target_bundle.gene_names.astype(str)[gene_order])

    dataset = ExprDataset(ordered_encoder, ordered_log, module_targets, labels)
    ...
    model = AudioMAEModulePatchScMAE(
        num_genes=ordered_encoder.shape[1],
        ...
        module_target_dim=module_targets.shape[2],
    ).to(device)
    criterion = AudioMAEModulePatchLoss(args.masked_data_weight, args.mask_weight, args.module_weight, args.consistency_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "module_loss": [], "consistency_loss": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    print(f"Using device: {device}")
    print(f"Dataset={dataset_name} cells={ordered_encoder.shape[0]} genes={ordered_encoder.shape[1]} clusters={n_clusters}")
    print(f"Method={METHOD_NAME} stage={stage} epochs={args.epochs}")

    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "module_loss", "consistency_loss"]}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, module_cpu, _ in train_loader:
            x = x_cpu.to(device)
            target = log_cpu.to(device)
            module_target = module_cpu.to(device)
            out_a = model(x, args.mask_ratio, args.structured_prob)
            out_b = model(x, max(0.2, args.mask_ratio * 0.65), min(0.8, args.structured_prob + 0.2))
            loss, parts = criterion(out_a, out_b, target, module_target)
            ...
            for key in sums:
                sums[key] += parts[key]
            mask_sum += float(out_a["gene_mask"].mean().detach().cpu())
            n_batches += 1
        for key in sums:
    ...
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args), "gene_order": gene_order.astype(np.int64), "gene_names": target_bundle.gene_names.astype(str)[gene_order]}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log_expr ordered module-patch targets"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, float(np.var(module_targets)))
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_audiomae_module_patch_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_audiomae_module_patch_scmae.h5ad", compression="gzip")
