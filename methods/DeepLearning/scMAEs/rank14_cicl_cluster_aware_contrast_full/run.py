...
from loss import CICLClusterAwareLoss
from model import CICLClusterScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank14_cicl_cluster_aware_contrast_full"
DISPLAY_NAME = "scMAE + CICL cluster-aware contrast"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent CICL cluster-aware contrastive scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--instance_weight", type=float, default=0.08)
    parser.add_argument("--cluster_weight", type=float, default=0.06)
    ...
    parser.add_argument("--pseudo_update_interval", type=int, default=5)
    return parser.parse_args()


@torch.no_grad()
def extract_embedding(model: CICLClusterScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, q_all = [], [], []
    ...
    return np.nan_to_num(np.concatenate(emb).astype(np.float32)), np.concatenate(labels).astype(np.int64), np.concatenate(q_all).astype(np.float32)


def initialize_and_assign(model: CICLClusterScMAE, loader: DataLoader, n_clusters: int, seed: int, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    embedding, labels, _ = extract_embedding(model, loader, device)
    km = KMeans(n_clusters=n_clusters, n_init=20, random_state=seed).fit(embedding)
    model.cluster_centers.data.copy_(torch.as_tensor(km.cluster_centers_, dtype=torch.float32, device=device))
    with torch.no_grad():
        q_chunks = []
        model.eval()
        for _, x, _, _ in loader:
            q_chunks.append(model(x.to(device))["cluster_q"].detach().cpu().numpy())
    q = np.concatenate(q_chunks).astype(np.float32)
    pseudo = q.argmax(axis=1).astype(np.int64)
    confidence = q.max(axis=1).astype(np.float32)
    return pseudo, confidence, labels
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, q: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, confidence_threshold: float) -> dict:
    if preds is None:
    ...
    confidence = q.max(axis=1) if q.size else np.array([])
    return {
        "edge_survival": 1.0,
        ...
        "cluster_confidence_mean": float(confidence.mean()) if confidence.size else 0.0,
        "cluster_confident_fraction": float(np.mean(confidence >= confidence_threshold)) if confidence.size else 0.0,
        "diagnostic_note": "No NeighborMix graph is used; CICL pseudo-label positives and false-negative filtering are used.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "contrast_target": "CICL warmup then KMeans pseudo-label positives with false-negative filtering"}, str(save_dir / "preprocess_config.json"))
    ...
    model = CICLClusterScMAE(encoder_data.shape[1], n_clusters, args.hidden_size, args.projection_dim, args.dropout).to(device)
    criterion = CICLClusterAwareLoss(args.masked_data_weight, args.mask_weight, args.instance_weight, args.cluster_weight, args.temperature, args.confidence_threshold)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    pseudo_labels: np.ndarray | None = None
    confidence: np.ndarray | None = None
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "instance_loss": [], "cluster_loss": [], "confidence_fraction": [], "cluster_valid_fraction": [], "effective_mask_rate": [], "contrast_scale": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        if epoch > args.warmup_epochs and ((epoch - args.warmup_epochs - 1) % max(1, args.pseudo_update_interval) == 0 or pseudo_labels is None):
            pseudo_labels, confidence, _ = initialize_and_assign(model, full_loader, n_clusters, args.seed + epoch, device)
        contrast_scale = 0.0 if epoch <= args.warmup_epochs else min(1.0, (epoch - args.warmup_epochs) / max(1, args.warmup_epochs))
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "instance_loss", "cluster_loss", "confidence_fraction", "cluster_valid_fraction"]}
        mask_sum = 0.0
        n_batches = 0
        for idx, x_cpu, log_cpu, _ in train_loader:
            x = x_cpu.to(device)
            target = log_cpu.to(device)
            v1, mask = model.random_mask_view(x, args.mask_prob, args.jitter_std)
            v2, _ = model.random_mask_view(x, max(0.05, args.mask_prob * 0.75), args.jitter_std)
            out1 = model(v1)
            out2 = model(v2)
            batch_pseudo = None
            batch_conf = None
            if pseudo_labels is not None and confidence is not None:
                ids = idx.numpy()
                batch_pseudo = torch.as_tensor(pseudo_labels[ids], dtype=torch.long, device=device)
                batch_conf = torch.as_tensor(confidence[ids], dtype=torch.float32, device=device)
            loss, parts = criterion(out1, out2, target, mask, batch_pseudo, batch_conf, contrast_scale)
        ...
        history["contrast_scale"].append(contrast_scale)
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} inst={history['instance_loss'][-1]:.4f} clu={history['cluster_loss'][-1]:.4f} conf={history['confidence_fraction'][-1]:.4f} scale={contrast_scale:.2f} mask_rate={history['effective_mask_rate'][-1]:.4f}")

    embedding, labels_out, q_out = extract_embedding(model, full_loader, device)
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + CICL cluster-aware contrast"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, q_out, n_clusters, args.seed, preds, args.confidence_threshold)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_cicl_cluster_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_cicl_cluster_scmae.h5ad", compression="gzip")
