...
from loss import AMPDRLMaskLoss
from model import AMPDRLMaskScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank49_amp_drl_mask_policy_full"
DISPLAY_NAME = "scMAE + AMP-DRL adaptive mask policy"
...
ACTIONS = [(mode, ratio) for mode in ("zero", "swap", "variance") for ratio in (0.25, 0.40, 0.55)]
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent AMP-DRL adaptive masking scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--policy_weight", type=float, default=0.02)
    p.add_argument("--variance_weight", type=float, default=0.0)
    p.add_argument("--epsilon_start", type=float, default=0.35)
    p.add_argument("--epsilon_end", type=float, default=0.05)
    return p.parse_args()


def build_state(x: torch.Tensor, epoch_frac: float, prev_loss: float, prev_reward: float) -> torch.Tensor:
    nonzero = (x != 0).float().mean()
    state = torch.stack([
        x.mean(),
        x.std(unbiased=False),
        nonzero,
        x.var(dim=1, unbiased=False).mean(),
        x.new_tensor(epoch_frac),
        x.new_tensor(prev_reward if np.isfinite(prev_reward) else 0.0),
    ])
    state[4] = state[4].clamp(0, 1)
    state[5] = state[5].clamp(-1, 1)
    if np.isfinite(prev_loss):
        state[3] = state[3] / (abs(prev_loss) + 1e-6)
    return torch.nan_to_num(state).unsqueeze(0)


def apply_policy_mask(x: torch.Tensor, action_id: int, gene_weights: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, str, float]:
    mode, ratio = ACTIONS[int(action_id)]
    if mode == "variance":
        w = gene_weights.to(x.device).float().clamp_min(1e-6)
        p = (ratio * w / w.mean().clamp_min(1e-6)).clamp(0.02, 0.90).view(1, -1)
    ...
    return corrupted, mask.float(), mode, float(ratio)


@torch.no_grad()
def extract_embedding(model: AMPDRLMaskScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, action_hist: dict[str, int]) -> dict:
    if preds is None:
...
def main() -> int:
    ...
    gene_weights = torch.as_tensor(np.var(log_expr, axis=0).astype(np.float32) + 1e-6)
    np.save(save_dir / "gene_names.npy", target.gene_names.astype(str))
    save_json(target.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "amp_drl_core": "dueling-Q controller chooses zero/swap/variance-guided mask and ratio; reward is reconstruction loss improvement"}, str(save_dir / "preprocess_config.json"))
    ...
    model = AMPDRLMaskScMAE(encoder_data.shape[1], len(ACTIONS), args.hidden_size, args.decoder_hidden, args.dropout).to(device)
    criterion = AMPDRLMaskLoss(args.masked_data_weight, args.mask_loss_weight, args.policy_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "policy_loss", "variance_loss", "effective_mask_rate", "reward"]
    history = {k: [] for k in keys}
    history["stage"] = stage
    action_counts = {f"{i}:{m}:{r}": 0 for i, (m, r) in enumerate(ACTIONS)}
    prev_loss, prev_reward = np.nan, 0.0
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        nb = 0
        eps = args.epsilon_end + (args.epsilon_start - args.epsilon_end) * max(0.0, 1.0 - (epoch - 1) / max(1, args.epochs - 1))
        for _, x_cpu, log_cpu, _ in train_loader:
            x, y = x_cpu.to(device), log_cpu.to(device)
            state = build_state(x, (epoch - 1) / max(1, args.epochs), prev_loss, prev_reward).to(device)
            q = model.policy(state)
            if torch.rand((), device=device).item() < eps:
                action = int(torch.randint(0, len(ACTIONS), (1,), device=device).item())
            else:
                action = int(q.argmax(dim=1).item())
            corrupted, mask, mode, ratio = apply_policy_mask(x, action, gene_weights)
            out = model(corrupted)
            with torch.no_grad():
                scmae_probe, probe_parts = criterion.scmae_loss(out, y, mask)
                current_loss = float(probe_parts["scmae_loss"])
                reward = 0.0 if not np.isfinite(prev_loss) else float(np.clip(prev_loss - current_loss, -1.0, 1.0))
                td_target = torch.tensor([reward], dtype=torch.float32, device=device)
            loss, parts = criterion(out, y, mask, q[:, action], td_target)
            ...
            opt.step()
            prev_loss, prev_reward = current_loss, reward
            action_counts[f"{action}:{mode}:{ratio}"] += 1
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} reward={history['reward'][-1]:.4f} mask={history['effective_mask_rate'][-1]:.3f} eps={eps:.3f}")
    ...
    save_json(action_counts, str(save_dir / "policy_action_histogram.json"))
    torch.save({"model": model.state_dict(), "optimizer": opt.state_dict(), "args": vars(args), "gene_names": target.gene_names.astype(str), "actions": ACTIONS}, save_dir / "model_checkpoint.pth")
    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + AMP-DRL adaptive mask policy"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], action_counts)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_amp_drl_mask_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_amp_drl_mask_scmae.h5ad", compression="gzip")
