...
from loss import ScVGAEZINBLoss
...
METHOD_NAME = "rank06_scvgae_zinb_variational_full"
DISPLAY_NAME = "scMAE + scVGAE ZINB variational decoder"
...
class CountDataset(Dataset):
    def __init__(self, encoder_data: np.ndarray, log_expr: np.ndarray, counts: np.ndarray, labels: np.ndarray):
        ...
        self.size_factor = torch.as_tensor(np.maximum(counts.sum(axis=1), 1.0), dtype=torch.float32)
    ...
    def __getitem__(self, idx: int):
        return int(idx), self.encoder_data[idx], self.log_expr[idx], self.counts[idx], self.size_factor[idx], self.labels[idx]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent scVGAE ZINB variational scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--zinb_weight", type=float, default=0.01)
    parser.add_argument("--kl_weight", type=float, default=0.001)
    return parser.parse_args()


def load_count_branch(data_path: str, input_mode: str, selected_genes: np.ndarray, target_sum: float) -> tuple[np.ndarray, dict]:
    adata = sc.read_h5ad(data_path)
    matrix, source_gene_names, _, counts_source, inferred_mode = family.select_count_source(adata, input_mode)
    counts = family.ensure_csr(matrix)
    source_gene_names = np.asarray(source_gene_names).astype(str)
    gene_to_idx = {gene: idx for idx, gene in enumerate(source_gene_names)}
    missing = [gene for gene in selected_genes.astype(str) if gene not in gene_to_idx]
    if missing:
        raise ValueError(f"Count branch is missing selected genes, first missing={missing[:5]}")
    indices = np.asarray([gene_to_idx[gene] for gene in selected_genes.astype(str)], dtype=np.int64)
    selected_counts = counts[:, indices]
    count_arr = family.dense_float32(selected_counts)
    if inferred_mode != "raw":
        count_arr = np.expm1(np.maximum(count_arr, 0.0)).astype(np.float32)
        count_source_note = "count_like_expm1_from_log1p_source"
    else:
        count_source_note = "raw_count_source"
    count_arr = np.nan_to_num(count_arr, nan=0.0, posinf=0.0, neginf=0.0)
    count_arr[count_arr < 0.0] = 0.0
    profile = {
        "counts_source": counts_source,
        "inferred_mode": inferred_mode,
        "count_source_note": count_source_note,
        "target_sum": float(target_sum),
        "selected_n_genes": int(len(selected_genes)),
        "mean_library_size": float(np.mean(count_arr.sum(axis=1))),
    }
    return count_arr.astype(np.float32), profile


@torch.no_grad()
def extract_embedding(model: ScVGAEZINBScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    embeddings, labels = [], []
    for _, x, _, _, _, y in loader:
...
def compute_diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, posterior_var: float, count_profile: dict) -> dict:
    if preds is None:
    ...
    return {
        ...
        "posterior_variance_mean": float(posterior_var),
        "count_branch": count_profile,
        "diagnostic_note": "No NeighborMix graph is used; ZINB uses non-scaled count/count-like branch.",
    }
...
def main() -> int:
    ...
    count_data, count_profile = load_count_branch(args.data_path, args.input_mode, target_bundle.gene_names, args.target_sum)
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "count_branch": count_profile}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_names.npy", target_bundle.gene_names.astype(str))

    dataset = CountDataset(encoder_data, log_expr, count_data, labels)
    ...
    model = ScVGAEZINBScMAE(encoder_data.shape[1], args.hidden_size, args.latent_dim, args.dropout).to(device)
    criterion = ScVGAEZINBLoss(args.masked_data_weight, args.mask_weight, args.zinb_weight, args.kl_weight)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "zinb_loss": [], "kl_loss": [], "effective_mask_rate": [], "posterior_variance": [], "stage": stage}
    start = time.time()
    print(f"Using device: {device}")
    print(f"Dataset={dataset_name} cells={encoder_data.shape[0]} genes={encoder_data.shape[1]} clusters={n_clusters} count_source={count_profile['count_source_note']}")
    print(f"Method={METHOD_NAME} stage={stage} epochs={args.epochs}")

    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "zinb_loss", "kl_loss"]}
        mask_sum = 0.0
        posterior_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, count_cpu, size_cpu, _ in train_loader:
            x = x_cpu.to(device)
            log_target = log_cpu.to(device)
            count_target = count_cpu.to(device)
            size_factor = size_cpu.to(device)
            corrupted, mask = model.corrupt(x, args.mask_prob)
            outputs = model(corrupted, size_factor)
            loss, parts = criterion(outputs, log_target, count_target, mask)
            ...
            posterior_sum += float(outputs["logvar"].exp().mean().detach().cpu())
            n_batches += 1
        for key in sums:
            history[key].append(sums[key] / max(1, n_batches))
        history["effective_mask_rate"].append(mask_sum / max(1, n_batches))
        history["posterior_variance"].append(posterior_sum / max(1, n_batches))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} zinb={history['zinb_loss'][-1]:.4f} kl={history['kl_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args), "gene_names": target_bundle.gene_names.astype(str), "count_profile": count_profile}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log_expr rec + count ZINB", "count_source": count_profile["count_source_note"]})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))

    diag = compute_diagnostics(embedding, labels_out, n_clusters, args.seed, preds, history["posterior_variance"][-1], count_profile)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_scvgae_zinb_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_scvgae_zinb_scmae.h5ad", compression="gzip")
