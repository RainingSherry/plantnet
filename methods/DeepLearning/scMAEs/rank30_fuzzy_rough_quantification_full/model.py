...
class FuzzyRoughQuantScMAE(nn.Module):
    """scMAE with fuzzy rough lower/upper membership approximations."""
    ...
    def forward(self, x: torch.Tensor, anchor: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        ...
        entropy = -(q * q.clamp_min(1e-8).log()).sum(dim=1)
        max_entropy = torch.log(torch.tensor(float(self.n_clusters), device=q.device)).clamp_min(1e-8)
        certainty = (1.0 - entropy / max_entropy).clamp(0.0, 1.0)
        lower = q * certainty[:, None]
        upper = q + (1.0 - certainty[:, None]) * (1.0 - q)
        ...
        return {
            ...
            "rough_lower": lower,
            "rough_upper": upper,
            "rough_width": (upper - lower).mean(dim=1),
            "rough_certainty": certainty,
