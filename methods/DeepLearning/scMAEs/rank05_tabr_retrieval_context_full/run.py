...
from loss import TabRContextLoss
from model import TabRContextScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank05_tabr_retrieval_context_full"
DISPLAY_NAME = "scMAE + TabR retrieval context"
...
class RetrievalDataset(Dataset):
    def __init__(self, encoder_data: np.ndarray, context_data: np.ndarray, log_expr: np.ndarray, labels: np.ndarray):
        self.encoder_data = torch.as_tensor(encoder_data, dtype=torch.float32)
        self.context_data = torch.as_tensor(context_data, dtype=torch.float32)
    ...
    def __getitem__(self, idx: int):
        return int(idx), self.encoder_data[idx], self.context_data[idx], self.log_expr[idx], self.labels[idx]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent TabR-style retrieval-context scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--retrieval_svd_dim", type=int, default=32)
...
def build_context(log_expr: np.ndarray, encoder_data: np.ndarray, k: int, svd_dim: int, seed: int) -> tuple[np.ndarray, dict]:
    del seed
    n_components = min(int(svd_dim), log_expr.shape[1])
    if n_components >= 1:
        reduced = log_expr[:, :n_components]
    else:
        reduced = log_expr
    nn = NearestNeighbors(n_neighbors=min(int(k) + 1, log_expr.shape[0]), metric="euclidean")
    nn.fit(reduced)
    distances, indices = nn.kneighbors(reduced, return_distance=True)
    distances = distances[:, 1:]
    indices = indices[:, 1:]
    weights = 1.0 / (distances + 1e-6)
    weights = weights / weights.sum(axis=1, keepdims=True)
    context = np.einsum("nk,nkg->ng", weights.astype(np.float32), encoder_data[indices].astype(np.float32))
    diag = {
        "retrieval_k": int(indices.shape[1]),
        "retrieval_projection": "first_hvg_columns_no_svd",
        "retrieval_projection_dim": int(n_components),
        "mean_neighbor_distance": float(np.mean(distances)),
        "max_neighbor_distance": float(np.max(distances)),
    }
    return context.astype(np.float32), diag


@torch.no_grad()
def extract_embedding(model: TabRContextScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
    for _, x, ctx, _, y in loader:
        emb.append(model.feature(x.to(device), ctx.to(device)).detach().cpu().numpy())
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, retrieval_diag: dict) -> dict:
    if preds is None:
    ...
    return {
        ...
        "diagnostic_note": "Retrieval context is used during both training and inference; no NeighborMix cell mixing.",
    }
...
def main() -> int:
    ...
    context_data, retrieval_diag = build_context(log_expr, np.asarray(encoder_data, dtype=np.float32), args.retrieval_k, args.retrieval_svd_dim, args.seed)
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "retrieval_context": retrieval_diag}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_names.npy", target_bundle.gene_names.astype(str))
    np.save(save_dir / "retrieval_context.npy", context_data.astype(np.float32))

    dataset = RetrievalDataset(encoder_data, context_data, log_expr, labels)
    ...
    model = TabRContextScMAE(encoder_data.shape[1], args.hidden_size, args.dropout).to(device)
    criterion = TabRContextLoss(args.masked_data_weight, args.mask_weight, args.retrieval_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "reconstruction_loss": [], "mask_loss": [], "retrieval_loss": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    print(f"Using device: {device}")
    print(f"Dataset={dataset_name} cells={encoder_data.shape[0]} genes={encoder_data.shape[1]} clusters={n_clusters} retrieval_k={retrieval_diag['retrieval_k']}")
    print(f"Method={METHOD_NAME} stage={stage} epochs={args.epochs}")

    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {"loss": 0.0, "reconstruction_loss": 0.0, "mask_loss": 0.0, "retrieval_loss": 0.0}
        mask_sum, n_batches = 0.0, 0
        for _, x_cpu, ctx_cpu, target_cpu, _ in train_loader:
            x = x_cpu.to(device)
            ctx = ctx_cpu.to(device)
            target = target_cpu.to(device)
            corrupted, mask = model.corrupt_swap(x, args.mask_prob)
            outputs = model(corrupted, ctx)
            loss, parts = criterion(outputs, target, mask)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} rec={history['reconstruction_loss'][-1]:.4f} mask={history['mask_loss'][-1]:.4f} ret={history['retrieval_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args), "gene_names": target_bundle.gene_names.astype(str), "retrieval": retrieval_diag}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + retrieval context", "retrieval_k": int(retrieval_diag["retrieval_k"])})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, retrieval_diag)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_tabr_context_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_tabr_context_scmae.h5ad", compression="gzip")
