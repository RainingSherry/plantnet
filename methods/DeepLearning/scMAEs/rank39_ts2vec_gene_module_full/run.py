...
from loss import TS2VecGeneModuleLoss
from model import TS2VecGeneModuleScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank39_ts2vec_gene_module_full"
DISPLAY_NAME = "scMAE + TS2Vec gene-module contextual consistency"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent TS2Vec gene-module scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--module_dim", type=int, default=96)
    parser.add_argument("--module_size", type=int, default=25)
    parser.add_argument("--module_depth", type=int, default=4)
    ...
    parser.add_argument("--contrast_weight", type=float, default=0.04)
...
@torch.no_grad()
def extract_embedding(model: TS2VecGeneModuleScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, last_history: dict, n_modules: int) -> dict:
    ...
    return {
        ...
        "contrast_loss": float(last_history.get("contrast_loss", float("nan"))),
        "module_loss": float(last_history.get("module_loss", float("nan"))),
        "view_steps": float(last_history.get("view_steps", float("nan"))),
        "diagnostic_note": "No NeighborMix is used. TS2Vec is applied on an SVD-derived gene-module axis, not raw HVG index order.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "gene_module_order": order_info, "module_size": int(args.module_size), "n_modules": n_modules}, str(save_dir / "preprocess_config.json"))
    ...
    save_json({"gene_order": order_info, "module_size": int(args.module_size), "n_modules": n_modules, "note": "Gene modules are contiguous blocks after sampled log-expression SVD ordering."}, str(save_dir / "module_summary.json"))
    ...
    criterion = TS2VecGeneModuleLoss(args.masked_data_weight, args.mask_weight, args.module_weight, args.contrast_weight, args.variance_weight, args.contrast_alpha, args.temporal_unit)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "module_loss", "contrast_loss", "variance_loss", "view_steps", "module_repr_norm"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            view1, view2 = model.sample_context_views(x, args.temporal_unit)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} module={history['module_loss'][-1]:.4f} contrast={history['contrast_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression SVD gene-module TS2Vec consistency"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_ts2vec_gene_module_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_ts2vec_gene_module_scmae.h5ad", compression="gzip")
