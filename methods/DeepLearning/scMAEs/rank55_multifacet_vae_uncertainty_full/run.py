...
from loss import MultiFacetVAELoss
from model import MultiFacetVAEscMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank55_multifacet_vae_uncertainty_full"
DISPLAY_NAME = "scMAE + multi-facet VAE uncertainty"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent multi-facet VAE uncertainty scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--n_facets", type=int, default=2)
    p.add_argument("--facet_dim", type=int, default=32)
    ...
    p.add_argument("--nb_weight", type=float, default=0.02)
    p.add_argument("--entropy_weight", type=float, default=0.002)
...
def load_aligned_raw_counts(data_path: str, input_mode: str, target_gene_names: np.ndarray) -> tuple[np.ndarray | None, np.ndarray | None, str]:
    ...
    source_x, gene_names, var, source_desc, inferred_mode = family.select_count_source(adata, input_mode)
    ...
    size_factor = np.clip(size_factor, 1e-3, 1e3).astype(np.float32)
    return raw.astype(np.float32), size_factor, f"aligned raw counts from {source_desc}"
...
@torch.no_grad()
def extract_embedding(model: MultiFacetVAEscMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    ...
    for _, x_cpu, _, y, _, _ in loader:
        ...
        pv = torch.stack([torch.exp(v).mean(dim=1) for v in out["logvars"]], dim=1).mean(dim=1)
        post_var.append(pv.detach().cpu().numpy())
    return np.nan_to_num(np.concatenate(emb).astype(np.float32)), np.concatenate(labels).astype(np.int64), np.nan_to_num(np.concatenate(mask_scores).astype(np.float32)), np.nan_to_num(np.concatenate(post_var).astype(np.float32))
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, post_var: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, raw_available: bool) -> dict:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix is used. NB loss uses aligned raw counts only when available; scaled expression is never used as count data.",
    }
...
def main() -> int:
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "raw_count_branch": raw_note, "rank55_core": "MFCVAE multi-facet latent MoG with optional raw-count NB likelihood"}, str(save_dir / "preprocess_config.json"))

    dataset = MultiFacetDataset(encoder_data, log_expr, labels, raw_counts, size_factor)
    ...
    model = MultiFacetVAEscMAE(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, args.n_facets, args.facet_dim, args.n_components, args.dropout).to(device)
    criterion = MultiFacetVAELoss(args.masked_data_weight, args.mask_loss_weight, args.kl_weight, args.nb_weight if raw_available else 0.0, args.entropy_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "kl_loss", "nb_loss", "facet_entropy", "variance_loss", "effective_mask_rate"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _, raw_cpu, sf_cpu in train_loader:
            ...
            out = model(corrupted)
            loss, parts, _ = criterion(model, out, y, mask, raw, sf)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} kl={history['kl_loss'][-1]:.4f} nb={history['nb_loss'][-1]:.4f} mask={history['effective_mask_rate'][-1]:.3f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + optional aligned raw-count NB + multi-facet VAE"})
    ...
    if not args.no_save_h5ad:
        enc.adata.obsm["X_multifacet_vae_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_multifacet_vae_scmae.h5ad", compression="gzip")
