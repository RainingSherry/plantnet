...
from loss import GroundGANCausalMaskLoss
from model import GroundGANCausalMaskScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank64_groundgan_causal_mask_full"
DISPLAY_NAME = "scMAE + GRouNdGAN causal mask"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent GRouNdGAN causal-mask scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--causal_hidden", type=int, default=128)
    p.add_argument("--dropout", type=float, default=0.0)
    p.add_argument("--mask_prob", type=float, default=0.4)
    p.add_argument("--replace_prob", type=float, default=1.0)
    p.add_argument("--regulator_k", type=int, default=8)
    p.add_argument("--target_batch", type=int, default=128)
    p.add_argument("--tf_fraction", type=float, default=0.25)
    p.add_argument("--regulator_dropout", type=float, default=0.2)
    p.add_argument("--masked_data_weight", type=float, default=0.75)
    p.add_argument("--mask_loss_weight", type=float, default=0.7)
    p.add_argument("--causal_weight", type=float, default=0.05)
    p.add_argument("--regulator_dropout_weight", type=float, default=0.005)
...
def build_causal_mask(log_expr: np.ndarray, regulator_k: int, tf_fraction: float, seed: int) -> tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
    x = np.asarray(log_expr, dtype=np.float32)
    gene_var = x.var(axis=0)
    n_genes = x.shape[1]
    n_tfs = int(np.clip(round(n_genes * float(tf_fraction)), regulator_k + 1, n_genes))
    tf_pool = np.argsort(gene_var)[-n_tfs:].astype(np.int64)
    xz = x - x.mean(axis=0, keepdims=True)
    xz = xz / (x.std(axis=0, keepdims=True) + 1e-6)
    tf_x = xz[:, tf_pool]
    corr = (xz.T @ tf_x) / max(1, x.shape[0] - 1)
    weights = np.abs(corr)
    reg_idx = np.zeros((n_genes, regulator_k), dtype=np.int64)
    reg_w = np.zeros((n_genes, regulator_k), dtype=np.float32)
    for g in range(n_genes):
        score = weights[g].copy()
        score[tf_pool == g] = -np.inf
        top = np.argpartition(score, -regulator_k)[-regulator_k:]
        top = top[np.argsort(score[top])[::-1]]
        reg_idx[g] = tf_pool[top]
        vals = np.maximum(score[top], 0.0).astype(np.float32)
        reg_w[g] = vals / (vals.sum() + 1e-6)
    stats = {
        "tf_pool_size": int(len(tf_pool)),
        "regulator_k": int(regulator_k),
        "tf_fraction": float(tf_fraction),
        "causal_edge_count": int(n_genes * regulator_k),
        "causal_weight_mean": float(reg_w.mean()),
        "causal_weight_std": float(reg_w.std()),
    }
    return tf_pool.astype(np.int64), reg_idx, reg_w, stats


@torch.no_grad()
def extract_embedding(model: GroundGANCausalMaskScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, causal_loss: float, causal_stats: dict) -> dict:
    if preds is None:
    ...
    return {
        ...
        "causal_loss_last": float(causal_loss),
        "tf_pool_size": int(causal_stats.get("tf_pool_size", 0)),
        "causal_edge_count": int(causal_stats.get("causal_edge_count", 0)),
        "input_mask_rate": float(mask_rate),
        "diagnostic_note": "No NeighborMix is used. GRouNdGAN is adapted as masked causal regulator-target consistency only.",
    }
...
def main() -> int:
    ...
    if args.smoke:
        ...
        args.target_batch = min(args.target_batch, 64)
    ...
    tf_pool, regulator_idx, regulator_w, causal_stats = build_causal_mask(log_expr, args.regulator_k, args.tf_fraction, args.seed)
    np.save(save_dir / "gene_names.npy", target.gene_names.astype(str))
    np.save(save_dir / "pseudo_tf_pool.npy", tf_pool.astype(np.int64))
    np.save(save_dir / "causal_regulators.npy", regulator_idx.astype(np.int64))
    np.save(save_dir / "causal_regulator_weights.npy", regulator_w.astype(np.float32))
    save_json(target.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "rank64_core": "GRouNdGAN-style masked causal regulator-target consistency", **causal_stats}, str(save_dir / "preprocess_config.json"))
    dataset = CausalMaskDataset(encoder_data, log_expr, labels)
    ...
    model = GroundGANCausalMaskScMAE(encoder_data.shape[1], args.regulator_k, args.hidden_size, args.decoder_hidden, args.gene_dim, args.causal_hidden, args.dropout).to(device)
    criterion = GroundGANCausalMaskLoss(args.masked_data_weight, args.mask_loss_weight, args.causal_weight, args.regulator_dropout_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    reg_idx_t = torch.as_tensor(regulator_idx, dtype=torch.long, device=device)
    reg_w_t = torch.as_tensor(regulator_w, dtype=torch.float32, device=device)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "causal_loss", "regulator_dropout_loss", "variance_loss", "effective_mask_rate"]
    history = {k: [] for k in keys}
    history["stage"] = stage
    start = time.time()
    print(f"Using device: {device}")
    print(f"Dataset={dataset_name} cells={encoder_data.shape[0]} genes={encoder_data.shape[1]} clusters={n_clusters} causal_edges={causal_stats['causal_edge_count']}")
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            out = model(corrupted)
            targets = torch.randint(0, encoder_data.shape[1], (min(args.target_batch, encoder_data.shape[1]),), device=device)
            regs = reg_idx_t[targets]
            reg_w = reg_w_t[targets]
            reg_values = y[:, regs]
            pred = model.causal_predict(out["embedding"], targets, reg_values, reg_w)
            causal_target = y[:, targets]
            drop_mask = (torch.rand_like(reg_values) > args.regulator_dropout).float()
            pred_dropout = model.causal_predict(out["embedding"], targets, reg_values * drop_mask, reg_w)
            loss, parts = criterion(out, y, mask, pred, causal_target, pred_dropout)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} causal={history['causal_loss'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + causal mask consistency"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], history["causal_loss"][-1], causal_stats)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_groundgan_causal_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_groundgan_causal_scmae.h5ad", compression="gzip")
