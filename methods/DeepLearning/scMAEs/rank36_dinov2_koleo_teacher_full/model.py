...
class DINOv2KoLeoScMAE(nn.Module):
    """DINOv2-style EMA teacher with class/patch distillation and scMAE heads."""
    ...
    def __init__(
        ...
        self.n_patches = (self.num_genes + self.patch_size - 1) // self.patch_size
        self.padded_genes = self.n_patches * self.patch_size
        self.student = DINOv2Branch(num_genes, patch_size, hidden_size, cls_out_dim, patch_out_dim, n_layers, num_heads, dropout)
        self.teacher = DINOv2Branch(num_genes, patch_size, hidden_size, cls_out_dim, patch_out_dim, n_layers, num_heads, dropout)
        self.reset_teacher()
