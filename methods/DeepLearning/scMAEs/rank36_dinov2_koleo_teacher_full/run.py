...
from loss import DINOv2KoLeoLoss
from model import DINOv2KoLeoScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank36_dinov2_koleo_teacher_full"
DISPLAY_NAME = "scMAE + DINOv2 KoLeo teacher"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent DINOv2 KoLeo teacher scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--koleo_weight", type=float, default=0.02)
...
@torch.no_grad()
def extract_embedding(model: DINOv2KoLeoScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix is used. DINOv2-style EMA teacher distillation plus KoLeo spreading is applied.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "dinov2_target": "EMA teacher centered/sharpened class and masked gene-module probability distributions plus KoLeo spreading", "neighbor_mix": "not used"}, str(save_dir / "preprocess_config.json"))
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} cls={history['cls_distill_loss'][-1]:.4f} patch={history['patch_distill_loss'][-1]:.4f} koleo={history['koleo_loss'][-1]:.4f} m={history['ema_momentum'][-1]:.5f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + DINOv2 KoLeo teacher"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_dinov2_koleo_teacher_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_dinov2_koleo_teacher_scmae.h5ad", compression="gzip")
