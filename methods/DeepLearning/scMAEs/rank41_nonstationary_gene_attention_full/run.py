...
from loss import NonStationaryGeneAttentionLoss
from model import NonStationaryGeneAttentionScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank41_nonstationary_gene_attention_full"
DISPLAY_NAME = "scMAE + non-stationary gene-module attention"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent non-stationary gene-module attention scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--projector_hidden", type=int, default=64)
    ...
    parser.add_argument("--factor_weight", type=float, default=0.001)
...
@torch.no_grad()
def extract_embedding(model: NonStationaryGeneAttentionScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, last_history: dict, n_modules: int) -> dict:
    ...
    return {
        ...
        "tau_mean": float(last_history.get("tau_mean", float("nan"))),
        "delta_abs_mean": float(last_history.get("delta_abs_mean", float("nan"))),
        "diagnostic_note": "No NeighborMix is used. Non-stationary attention is applied on an SVD-derived gene-module axis, not raw HVG index order.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "gene_module_order": order_info, "module_size": int(args.module_size), "n_modules": n_modules, "attention": "series stationarization with learned de-stationary tau/delta"}, str(save_dir / "preprocess_config.json"))
    ...
    save_json({"gene_order": order_info, "module_size": int(args.module_size), "n_modules": n_modules, "note": "Gene modules are contiguous blocks after sampled log-expression SVD ordering; non-stationary attention runs on these modules."}, str(save_dir / "module_summary.json"))
    ...
    model = NonStationaryGeneAttentionScMAE(
    ...
    criterion = NonStationaryGeneAttentionLoss(args.masked_data_weight, args.mask_weight, args.module_weight, args.consistency_weight, args.variance_weight, args.factor_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "module_loss", "consistency_loss", "variance_loss", "factor_loss", "tau_mean", "delta_abs_mean", "module_repr_norm"]
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression SVD gene-module non-stationary attention"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_nonstationary_gene_attention_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_nonstationary_gene_attention_scmae.h5ad", compression="gzip")
