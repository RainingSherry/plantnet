...
from loss import IJEPAScMAELoss
from model import IJEPAScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank22_ijepa_context_target_full"
DISPLAY_NAME = "scMAE + I-JEPA context-target"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent I-JEPA context-target scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--predictor_size", type=int, default=96)
    parser.add_argument("--predictor_depth", type=int, default=3)
    parser.add_argument("--num_heads", type=int, default=4)
    parser.add_argument("--dropout", type=float, default=0.05)
    parser.add_argument("--target_fraction", type=float, default=0.18)
    parser.add_argument("--n_targets", type=int, default=2)
    parser.add_argument("--context_fraction", type=float, default=0.72)
    parser.add_argument("--ema_base", type=float, default=0.996)
    parser.add_argument("--masked_data_weight", type=float, default=0.75)
    parser.add_argument("--mask_weight", type=float, default=0.55)
    parser.add_argument("--jepa_weight", type=float, default=0.18)
...
def coexpression_gene_order(log_expr: np.ndarray, n_anchors: int = 12, max_cells: int = 5000, seed: int = 42) -> np.ndarray:
    ...
    return np.lexsort((-strength, assign)).astype(np.int64)


@torch.no_grad()
def extract_embedding(model: IJEPAScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, target_fraction: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "target_patch_fraction": float(target_fraction),
        "diagnostic_note": "No NeighborMix graph is used; EMA target encoder supplies JEPA patch representations.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "gene_order_source": "coexpression anchor order from log_expr", "jepa_target_source": "EMA target encoder patch embeddings from clean ordered input"}, str(save_dir / "preprocess_config.json"))
    ...
    dataset = ExprDataset(ordered_encoder, ordered_log, labels)
    ...
    model = IJEPAScMAE(ordered_encoder.shape[1], args.patch_size, args.hidden_size, args.encoder_layers, args.predictor_size, args.predictor_depth, args.num_heads, args.dropout).to(device)
    criterion = IJEPAScMAELoss(args.masked_data_weight, args.mask_weight, args.jepa_weight, args.variance_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "jepa_loss": [], "variance_loss": [], "effective_mask_rate": [], "ema_momentum": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "jepa_loss", "variance_loss"]}
        mask_sum = 0.0
        mom_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            masks = model.sample_masks(x.shape[0], x.device, args.target_fraction, args.n_targets, args.context_fraction)
            out = model(x, masks)
            ...
            optimizer.step()
            step += 1
            momentum = 1.0 - (1.0 - args.ema_base) * (math.cos(math.pi * step / total_steps) + 1.0) / 2.0
            model.update_target_encoder(momentum)
            for key in sums:
                sums[key] += parts[key]
            mask_sum += float(out["gene_mask"].mean().detach().cpu())
            mom_sum += float(momentum)
            n_batches += 1
        for key in sums:
            history[key].append(sums[key] / max(1, n_batches))
        history["effective_mask_rate"].append(mask_sum / max(1, n_batches))
        history["ema_momentum"].append(mom_sum / max(1, n_batches))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} jepa={history['jepa_loss'][-1]:.4f} var={history['variance_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f} ema={history['ema_momentum'][-1]:.5f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log_expr ordered JEPA target encoder"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, args.target_fraction)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_ijepa_context_target_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_ijepa_context_target_scmae.h5ad", compression="gzip")
