...
class TrainLoop:
    ...
    def forward_backward(self, batch):
        ...
        for i in range(0, batch['feature'].shape[0], self.microbatch):
            ...
            micro_cond = {
                'cell_type': batch['cell_type'][i: i + self.microbatch].to(dist_util.dev()),
            }
            if self.source_model is False:
                micro_cond['ctrl_mean']= batch['ctrl_mean'][i: i + self.microbatch].to(dist_util.dev())
                micro_cond['ctrl_var']=batch['ctrl_var'][i: i + self.microbatch].to(dist_util.dev())

                if self.pert_type=="gene":
                    micro_cond['knockout'] = batch['knockout'][i: i + self.microbatch].to(dist_util.dev())
                else:
                    micro_cond['mole'] = batch['mole'][i: i + self.microbatch].to(dist_util.dev())
                    micro_cond['dosage'] = batch['dosage'][i: i + self.microbatch].to(dist_util.dev())
    ...
    def save(self):
        def save_checkpoint(rate, params):
            state_dict = self.mp_trainer.master_params_to_state_dict(
                params) if self.mp_trainer else self.model.state_dict()
