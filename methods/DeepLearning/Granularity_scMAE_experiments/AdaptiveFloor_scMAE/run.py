...
from loss import AdaptiveFloorLoss
...
METHOD_NAME = "AdaptiveFloor_scMAE"
DISPLAY_NAME = "scMAE + DEC + theory-driven adaptive variance floor"
BASELINES = {
...
def parse_args():
    ...
    p.add_argument("--floor_weight", type=float, default=0.02)
    ...
    p.add_argument("--floor_mode", default="ref", choices=["ref", "fixed", "none"])
    p.add_argument("--floor_scale", type=float, default=1.0)
    return p.parse_args()
...
def main() -> int:
    ...
    criterion = AdaptiveFloorLoss(args.masked_data_weight, args.mask_weight, args.cluster_weight,
                                  args.consistency_weight, args.floor_weight, args.confidence_threshold,
                                  args.floor_mode, args.floor_scale)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)

    p_targets = None
    centers_initialized = False
    ref_captured = False
    hist = {k: [] for k in ["loss", "scmae_loss", "cluster_loss", "consistency_loss", "floor_loss"]}
    hist["stage"] = stage
    start = time.time()
    print(f"Device={device} dataset={dataset_name} cells={encoder_data.shape[0]} clusters={n_clusters} floor_mode={args.floor_mode} floor_scale={args.floor_scale}")
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        if epoch > args.warmup_epochs and ((epoch - args.warmup_epochs - 1) % max(1, args.target_update_interval) == 0 or p_targets is None):
            ...
            if not centers_initialized:
                ...
                if args.floor_mode == "ref" and not ref_captured:
                    ref_std = torch.as_tensor(emb.std(axis=0).astype(np.float32))
                    criterion.set_reference(ref_std)
                    ref_captured = True
                    ed = eff_dim(emb)
                    print(f"  [REF CAPTURED] post-warmup per-dim std mean={ref_std.mean():.3f} min/max={ref_std.min():.3f}/{ref_std.max():.3f} eff_dim={ed:.1f}/{emb.shape[1]}")
            p_targets = model.sharpen(torch.as_tensor(q_full)).numpy().astype(np.float32)

        cluster_scale = 0.0 if epoch <= args.warmup_epochs else min(1.0, (epoch - args.warmup_epochs) / max(1, args.warmup_epochs))
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "cluster_loss", "consistency_loss", "floor_loss"]}
        nb = 0
        for idx, x_cpu, log_cpu, _ in train_loader:
            ...
            weak, _ = model.random_mask(x, args.mask_prob * 0.5)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={hist['loss'][-1]:.4f} scmae={hist['scmae_loss'][-1]:.4f} cluster={hist['cluster_loss'][-1]:.4f} floor={hist['floor_loss'][-1]:.4f} scale={cluster_scale:.2f}")

    embedding, q_out, labels_out = extract_all(model, full_loader, device)
    ed_final = eff_dim(embedding)
    ...
    save_json({"dataset": dataset_name, "method": DISPLAY_NAME, "stage": stage, "seed": int(args.seed), "n_clusters": int(n_clusters), "floor_mode": args.floor_mode, "floor_scale": float(args.floor_scale), "eff_dim_final": ed_final, "runtime_seconds": float(time.time() - start), "fixed_metrics": eval_result["fixed"] if eval_result else {}, "baseline": baseline, "meets_screen_baseline_any": meets}, str(save_dir / "summary.json"))
    print(f"[RESULT] {dataset_name} NMI={nmi} ARI={ari} meets={meets} floor_mode={args.floor_mode} eff_dim={ed_final:.1f}/{embedding.shape[1]}")
    return 0
