...
# Add local dir first so local layers take precedence
...
class scDCC(nn.Module):
    ...
    def encodeBatch(self, X, batch_size=256, device='cpu'):
        encoded = []
        num = X.shape[0]
    ...
    def pretrain_autoencoder(self, x, X_raw, size_factor, batch_size=256, lr=0.001, epochs=400, ae_save=True, ae_weights='AE_weights.pth.tar', device='cpu'):
        self.to(device)
    ...
    def fit(self, X, X_raw, sf, ml_ind1=np.array([]), ml_ind2=np.array([]), cl_ind1=np.array([]), cl_ind2=np.array([]),
            ml_p=1., cl_p=1., y=None, lr=1., batch_size=256, num_epochs=10, update_interval=1, tol=1e-3, save_dir="", device='cpu'):
        '''X: tensor data'''
        self.to(device)
        self.zinb_loss = ZINBLoss()
        if device.type == 'cuda':
        ...
        data = self.encodeBatch(X, device=device)
        self.y_pred = kmeans.fit_predict(data.data.cpu().numpy())
        self.y_pred_last = self.y_pred
        self.mu.data.copy_(torch.Tensor(kmeans.cluster_centers_).to(device))
        if y is not None:
        ...
        for epoch in range(num_epochs):
            if epoch % update_interval == 0:
                latent = self.encodeBatch(X, device=device)
