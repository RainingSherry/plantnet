...
def cluster_acc(y_true, y_pred):
    ...
    for idx in range(y_pred.size):
        weights[y_pred[idx], y_true[idx]] += 1
    from scipy.optimize import linear_sum_assignment
    rows, cols = linear_sum_assignment(weights.max() - weights)
    return sum(weights[row, col] for row, col in zip(rows, cols)) * 1.0 / y_pred.size
...
class scDCC(nn.Module):
    ...
    def encodeBatch(self, X, batch_size=256):
        ...
        for batch_idx in range(num_batch):
            xbatch = X[batch_idx*batch_size : min((batch_idx+1)*batch_size, num)].to(device)
    ...
    def pretrain_autoencoder(self, x, X_raw, size_factor, batch_size=256, lr=0.001, epochs=400, ae_save=True, ae_weights='AE_weights.pth.tar'):
        device = _module_device(self)
        self.to(device)
        self.zinb_loss = self.zinb_loss.to(device)
    ...
    def fit(self, X, X_raw, sf, ml_ind1=np.array([]), ml_ind2=np.array([]), cl_ind1=np.array([]), cl_ind2=np.array([]), 
        ...
        sf = torch.tensor(sf, dtype=torch.float32, device=device)
        ...
        self.mu.data.copy_(torch.tensor(kmeans.cluster_centers_, dtype=torch.float32, device=device))
        if y is not None:
        ...
        for epoch in range(num_epochs):
            ...
            print("#Epoch %3d: Total: %.4f Clustering Loss: %.4f ZINB Loss: %.4f" % (
                epoch + 1,
                float((train_loss / num).detach().cpu()),
                float((cluster_loss_val / num).detach().cpu()),
                float((recon_loss_val / num).detach().cpu()),
            ))
