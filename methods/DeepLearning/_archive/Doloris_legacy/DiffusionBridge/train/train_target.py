...
from models.target_diffusion import TargetDiffusion
...
def train_target(args):
    ...
    model = TargetDiffusion(domain_dim=args.latent_dim, shared_dim=args.latent_dim, hidden_dim=args.hidden_dim, num_steps=args.diffusion_steps).to(device)
    ...
    for epoch in range(args.epochs):
        ...
        for (batch_z,) in loader:
            ...
            loss_dict = model.training_loss(batch_z, recon_weight=args.recon_weight, prior_weight=args.prior_weight)
            loss = loss_dict['loss']
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
            total_loss += loss.item() * batch_z.size(0)
        print(f'target epoch={epoch+1} loss={total_loss / len(dataset):.6f}')

    output_dir = Path(args.output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    torch.save({'state_dict': model.state_dict(), 'latent_dim': args.latent_dim}, output_dir / 'target.pt')
...
if __name__ == '__main__':
    ...
    parser.add_argument('--teacher-mode', type=str, default='pca_graph', choices=['pca_graph', 'pca'])
