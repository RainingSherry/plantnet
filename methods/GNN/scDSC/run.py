# -*- coding: utf-8 -*-
"""
Unified scDSC Model Interface for scBench
==========================================
...
class SDCN(nn.Module):
    ...
    def __init__(self, n_enc_1, n_enc_2, n_enc_3, n_dec_1, n_dec_2, n_dec_3,
                 n_input, n_z, n_clusters, pretrain_path=None, device='cuda', v=1):
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description='scDSC: Structural Deep Clustering for scRNA-seq',
    ...
    parser.add_argument('--n_enc_2', type=int, default=128,
                        help='Encoder layer 2 dimension')
    parser.add_argument('--n_enc_3', type=int, default=256,
...
def pretrain_ae(model, X, X_raw, sf, args, device):
    """Pretrain autoencoder."""
    optimizer = Adam(model.ae.parameters(), lr=args.pretrain_lr)
    X_tensor = torch.tensor(X).to(device)
    X_raw_tensor = torch.tensor(X_raw).to(device)
    sf_tensor = torch.tensor(sf).to(device)

    for epoch in range(args.pretrain_epochs):
        model.train()
        x_bar, _, _, _, z, dec_h3 = model.ae(X_tensor)
...
def main():
    ...
    model = pretrain_ae(model, X, raw_counts, sf, args, device)

    # Initialize cluster centers with KMeans
    X_tensor = torch.tensor(X).to(device)
    with torch.no_grad():
        _, _, _, _, z, _ = model.ae(X_tensor)
