...
from loss import BEiTGeneTokenLoss
from model import BEiTGeneTokenScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank18_beit_gene_token_full"
DISPLAY_NAME = "scMAE + BEiT gene-token target"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent BEiT gene-token scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
@torch.no_grad()
def extract_embedding(model: BEiTGeneTokenScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, replaced_fraction: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix graph is used; BEiT-style gene-specific discrete token prediction is used.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "token_source": "gene-specific log_expr quantile bins", "beit_corruption": "masked genes are zeroed or replaced by another cell for replaced-expression detection"}, str(save_dir / "preprocess_config.json"))
    ...
    model = BEiTGeneTokenScMAE(encoder_data.shape[1], args.hidden_size, args.token_bins, args.dropout).to(device)
    criterion = BEiTGeneTokenLoss(args.masked_data_weight, args.mask_weight, args.token_weight, args.replaced_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "token_loss": [], "replaced_loss": [], "replaced_fraction": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "token_loss", "replaced_loss", "replaced_fraction"]}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, token_cpu, _ in train_loader:
            ...
            corrupted, mask, replaced = model.beit_corrupt(x, args.mask_prob, args.replace_prob)
            out = model(corrupted)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} token={history['token_loss'][-1]:.4f} replaced={history['replaced_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + BEiT gene token target"})
    ...
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, replaced_fraction)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_beit_gene_token_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_beit_gene_token_scmae.h5ad", compression="gzip")
