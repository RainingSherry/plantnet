...
from loss import SAINTRowColLoss
from model import SAINTRowColScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank45_saint_rowcol_attention_full"
DISPLAY_NAME = "scMAE + SAINT row-column attention"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent SAINT row-column attention scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--token_count", type=int, default=64)
    p.add_argument("--heads", type=int, default=4)
    p.add_argument("--dropout", type=float, default=0.05)
    p.add_argument("--mask_prob", type=float, default=0.4)
    p.add_argument("--view_mask_prob", type=float, default=0.55)
    p.add_argument("--recon_weight", type=float, default=1.0)
    p.add_argument("--mask_weight", type=float, default=0.05)
    p.add_argument("--contrast_weight", type=float, default=0.05)
    p.add_argument("--gate_weight", type=float, default=0.005)
    p.add_argument("--variance_weight", type=float, default=0.01)
    return p.parse_args()


@torch.no_grad()
def extract_embedding(model: SAINTRowColScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, gates = [], [], []
    for _, x_cpu, _, y in loader:
        out = model(x_cpu.to(device))
        emb.append(out["embedding"].detach().cpu().numpy())
        gates.append(out["feature_gate"].mean(dim=1).detach().cpu().numpy())
        labels.append(y.numpy())
    return np.nan_to_num(np.concatenate(emb).astype(np.float32)), np.concatenate(labels).astype(np.int64), np.nan_to_num(np.concatenate(gates).astype(np.float32))
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, gates: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None) -> dict:
    if preds is None:
    ...
    return {"edge_survival": 1.0, "neighbor_purity_proxy": neighbor_purity(labels, embedding), "mixed_cell_fraction": 0.0, "boundary_entropy": float(-(probs * np.log(probs)).sum() / max(np.log(max(2, n_clusters)), 1e-8)), "rare_risk_fraction": float(np.mean([x in rare for x in labels])) if labels.size else 0.0, "embedding_variance": var, "cluster_mass_min": float(frac.min()) if frac.size else 0.0, "cluster_mass_max": float(frac.max()) if frac.size else 0.0, "collapse_warning": bool((not np.isfinite(var)) or var < 1e-8 or (frac.size and frac.min() < 0.001) or (frac.size and frac.max() > 0.95)), "edge_confidence_mean": float("nan"), "edge_confidence_std": float("nan"), "feature_gate_mean": float(np.mean(gates)) if gates.size else float("nan"), "diagnostic_note": "No NeighborMix is used. SAINT row-column attention uses feature tokens and batch row attention; no graph edges are trained."}
...
def main() -> int:
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "saint_core": "gene-group feature tokenizer + column attention + batch row attention"}, str(save_dir / "preprocess_config.json"))
    ...
    model = SAINTRowColScMAE(encoder_data.shape[1], args.token_count, args.hidden_size, args.decoder_hidden, args.heads, args.dropout).to(device)
    criterion = SAINTRowColLoss(args.recon_weight, args.mask_weight, args.contrast_weight, args.gate_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "recon_loss", "mask_loss", "contrast_loss", "gate_loss", "variance_loss", "effective_mask_rate"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            view, _ = model.mask_view(x, args.view_mask_prob)
            out = model(corrupted)
            with torch.no_grad():
                out2 = model(view)
            loss, parts = criterion(out, y, mask, out2["projection"])
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} recon={history['recon_loss'][-1]:.4f} contrast={history['contrast_loss'][-1]:.4f} gate={history['gate_loss'][-1]:.4f}")
    embedding, labels_out, gates = extract_embedding(model, full_loader, device)
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + SAINT row-column attention"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, gates, n_clusters, args.seed, preds)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_saint_rowcol_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_saint_rowcol_scmae.h5ad", compression="gzip")
