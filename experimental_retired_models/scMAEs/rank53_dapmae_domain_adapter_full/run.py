...
from loss import DAPMAELoss
from model import DAPMAEScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank53_dapmae_domain_adapter_full"
DISPLAY_NAME = "scMAE + DAP-MAE domain adapter"
...
class DAPDataset(Dataset):
    def __init__(self, encoder_data: np.ndarray, log_expr: np.ndarray, labels: np.ndarray, domain_id: np.ndarray, domain_target: np.ndarray):
        ...
        self.domain_id = torch.as_tensor(domain_id, dtype=torch.long)
        self.domain_target = torch.as_tensor(domain_target, dtype=torch.float32)

    def __len__(self) -> int:
        return int(self.encoder_data.shape[0])

    def __getitem__(self, idx: int):
        return int(idx), self.encoder_data[idx], self.log_expr[idx], self.labels[idx], self.domain_id[idx], self.domain_target[idx]


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent DAP-MAE domain-adapter scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--n_domains", type=int, default=3)
    p.add_argument("--mask_prob", type=float, default=0.4)
    p.add_argument("--replace_prob", type=float, default=0.35)
    p.add_argument("--masked_data_weight", type=float, default=0.75)
    p.add_argument("--mask_loss_weight", type=float, default=0.7)
    p.add_argument("--domain_ce_weight", type=float, default=0.08)
    p.add_argument("--domain_feature_weight", type=float, default=0.08)
    p.add_argument("--contrast_weight", type=float, default=0.02)
    p.add_argument("--variance_weight", type=float, default=0.0)
    return p.parse_args()


def make_domain_targets(log_expr: np.ndarray, hidden_size: int, n_domains: int, seed: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    use_dim = int(min(hidden_size, log_expr.shape[0] - 1, log_expr.shape[1] - 1))
    ...
    if use_dim < hidden_size:
        rep = np.pad(rep, ((0, 0), (0, hidden_size - use_dim)))
    n_domains = int(min(max(2, n_domains), rep.shape[0]))
    domain_id = KMeans(n_clusters=n_domains, n_init=20, random_state=seed).fit_predict(rep).astype(np.int64)
    proto = np.zeros((n_domains, hidden_size), dtype=np.float32)
    for d in range(n_domains):
        mask = domain_id == d
        proto[d] = rep[mask].mean(axis=0) if np.any(mask) else rep.mean(axis=0)
    target = proto[domain_id].astype(np.float32)
    return domain_id, target, proto
...
@torch.no_grad()
def extract_embedding(model: DAPMAEScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, mask_scores, domains = [], [], [], []
    for _, x_cpu, _, y, d, _ in loader:
        out = model(x_cpu.to(device), d.to(device))
    ...
    return np.nan_to_num(np.concatenate(emb).astype(np.float32)), np.concatenate(labels).astype(np.int64), np.nan_to_num(np.concatenate(mask_scores).astype(np.float32)), np.concatenate(domains).astype(np.int64)
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, domains: np.ndarray, mask_scores: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, n_domains: int) -> dict:
    if preds is None:
    ...
    dom = np.bincount(domains, minlength=n_domains).astype(np.float64)
    dom = dom / max(1.0, dom.sum())
    ...
    return {
        ...
        "pseudo_domain_entropy": float(-(dom[dom > 0] * np.log(dom[dom > 0])).sum() / max(np.log(max(2, n_domains)), 1e-8)),
        "n_pseudo_domains": int(n_domains),
        "diagnostic_note": "No NeighborMix is used. Pseudo-domain labels are unsupervised SVD/KMeans domains used only for DAP-MAE adapter and DFG targets.",
    }
...
def main() -> int:
    ...
    domain_id, domain_target, prototypes = make_domain_targets(log_expr, args.hidden_size, args.n_domains, args.seed)
    n_domains = int(prototypes.shape[0])
    np.save(save_dir / "gene_names.npy", target.gene_names.astype(str))
    np.save(save_dir / "pseudo_domain_id.npy", domain_id.astype(np.int64))
    np.save(save_dir / "domain_prototypes.npy", prototypes.astype(np.float32))
    save_json(target.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "rank53_core": "DAP-MAE heterogeneous pseudo-domain adapter and domain feature generator targets"}, str(save_dir / "preprocess_config.json"))

    dataset = DAPDataset(encoder_data, log_expr, labels, domain_id, domain_target)
    ...
    model = DAPMAEScMAE(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, n_domains, args.dropout).to(device)
    criterion = DAPMAELoss(args.masked_data_weight, args.mask_loss_weight, args.domain_ce_weight, args.domain_feature_weight, args.contrast_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "domain_ce_loss", "domain_feature_loss", "contrast_loss", "variance_loss", "effective_mask_rate"]
    history = {k: [] for k in keys}
    history["stage"] = stage
    start = time.time()
    print(f"Using device: {device}")
    print(f"Dataset={dataset_name} cells={encoder_data.shape[0]} genes={encoder_data.shape[1]} clusters={n_clusters} pseudo_domains={n_domains}")
    print(f"Method={METHOD_NAME} stage={stage} epochs={args.epochs}")
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in keys}
        nb = 0
        for _, x_cpu, log_cpu, _, d_cpu, tgt_cpu in train_loader:
            x, y, d, tgt = x_cpu.to(device), log_cpu.to(device), d_cpu.to(device), tgt_cpu.to(device)
            corrupted, mask = corrupt(x, args.mask_prob, args.replace_prob)
            out = model(corrupted, d)
            loss, parts, _ = criterion(out, y, mask, d, tgt)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} dfg={history['domain_feature_loss'][-1]:.4f} ce={history['domain_ce_loss'][-1]:.4f} mask={history['effective_mask_rate'][-1]:.3f}")

    embedding, labels_out, mask_scores, domains_out = extract_embedding(model, full_loader, device)
    ...
    torch.save({"model": model.state_dict(), "optimizer": opt.state_dict(), "args": vars(args), "gene_names": target.gene_names.astype(str), "domain_prototypes": prototypes.astype(np.float32)}, save_dir / "model_checkpoint.pth")
    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + pseudo-domain adapter/DFG"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, domains_out, mask_scores, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], n_domains)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_dapmae_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_dapmae_scmae.h5ad", compression="gzip")
