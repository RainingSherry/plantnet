...
from model import LowFreqGeneProgramScMAE
from loss import LowFreqGeneProgramLoss


METHOD_DIR = "suture09_lowfreq_gene_program_preservation_full"
METHOD_NAME = "scMAE + low-frequency gene-program preservation"
BASELINES = {
...
def parse_args():
    ...
    p.add_argument("--adapter_weight", type=float, default=0.04)
    p.add_argument("--program_bins", type=int, default=64)
    p.add_argument("--program_weight", type=float, default=0.05)
...
def build_lowfreq_program(log_expr: np.ndarray, n_bins: int) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    x = np.nan_to_num(log_expr.astype(np.float32), nan=0.0, posinf=0.0, neginf=0.0)
    n_genes = x.shape[1]
    n_bins = int(max(2, min(n_bins, n_genes)))
    gene_mean = x.mean(axis=0)
    order = np.argsort(gene_mean, kind="mergesort")
    bins = np.array_split(order, n_bins)
    program = np.zeros((x.shape[0], n_bins), dtype=np.float32)
    membership = np.full(n_genes, -1, dtype=np.int64)
    for i, genes in enumerate(bins):
        membership[genes] = i
        program[:, i] = x[:, genes].mean(axis=1)
    program_mean = program.mean(axis=0, keepdims=True)
    program_std = program.std(axis=0, keepdims=True) + 1e-6
    program = (program - program_mean) / program_std
    return program.astype(np.float32), order.astype(np.int64), membership.astype(np.int64)


@torch.no_grad()
def extract_embedding(model, loader, program_tensor, device):
    model.eval()
    emb, labels = [], []
    for idx, x, y in loader:
        idx = idx.to(device)
        z = model.feature(x.to(device), program_tensor[idx])
...
@torch.no_grad()
def collect_gate(model, loader, program_tensor, device):
    model.eval()
    gates = []
    for idx, x, _ in loader:
        idx = idx.to(device)
        out = model.encode_with_program(x.to(device), program_tensor[idx])
        gates.append(out["program_gate"].detach().cpu().numpy())
    return np.concatenate(gates, axis=0).astype(np.float32)


def diagnostics(embedding, labels, pred, gate):
    if pred is None:
    ...
    return {
        ...
        "program_gate_mean": float(gate.mean()),
        "program_gate_std": float(gate.std()),
        "diagnostic_note": "Low-frequency gene program is a coarse log-expression target; no graph propagation and no cell mixing are used.",
    }
...
def main():
    ...
    program, gene_order, gene_program_membership = build_lowfreq_program(target.data, args.program_bins)
    np.save(out / "lowfreq_program.npy", program)
    np.save(out / "lowfreq_gene_order.npy", gene_order)
    np.save(out / "lowfreq_gene_program_membership.npy", gene_program_membership)

    target_tensor = torch.as_tensor(target.data, dtype=torch.float32, device=device)
    program_tensor = torch.as_tensor(program, dtype=torch.float32, device=device)
    ...
    loss_fn = LowFreqGeneProgramLoss(program_weight=args.program_weight, target_gate=args.target_gate)
    ...
    print(f"Method={METHOD_DIR} stage={stage} epochs={args.epochs} program_bins={program.shape[1]}")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for idx, x, _ in loader:
            idx = idx.to(device)
            x = x.to(device)
            outputs = model(x, program_tensor[idx])
            loss, parts = loss_fn(outputs, target_tensor[idx], program_tensor[idx])
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={rec['total']:.4f} rec={rec['reconstruction']:.4f} mask={rec['mask_bce']:.4f} prog={rec['program_loss']:.4f} gate={rec['program_gate_mean']:.3f}")

    embedding, labels = extract_embedding(model, eval_loader, program_tensor, device)
    ...
    gate = collect_gate(model, eval_loader, program_tensor, device)
    np.save(out / "program_gate.npy", gate)
    fixed, pred = {}, None
    ...
    diag = diagnostics(embedding, labels, pred, gate)
