...
from loss import MaskedScClusterLoss
from model import MaskedScClusterScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank13_masked_sc_cluster_target_full"
DISPLAY_NAME = "scMAE + mask-sc cluster target"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent mask-sc cluster-target scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
@torch.no_grad()
def extract_embedding(model: MaskedScClusterScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    emb, labels, q_all = [], [], []
...
@torch.no_grad()
def refresh_targets(model: MaskedScClusterScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    emb, _, q = extract_embedding(model, loader, device)
    q_t = torch.as_tensor(q, dtype=torch.float32)
    p = MaskedScClusterScMAE.target_distribution(q_t).numpy().astype(np.float32)
    return emb, p


def initialize_centers(model: MaskedScClusterScMAE, embedding: np.ndarray, n_clusters: int, seed: int, device: torch.device) -> None:
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, q: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None) -> dict:
    if preds is None:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix graph is used; DEC-style high-confidence cluster target is used after scMAE warmup.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "cluster_target": "warmup then high-confidence DEC target distribution"}, str(save_dir / "preprocess_config.json"))
    ...
    model = MaskedScClusterScMAE(encoder_data.shape[1], n_clusters, args.hidden_size, args.dropout).to(device)
    criterion = MaskedScClusterLoss(args.masked_data_weight, args.mask_weight, args.cluster_weight, args.consistency_weight, args.confidence_threshold)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    p_targets: np.ndarray | None = None
    cluster_initialized = False
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "cluster_loss": [], "consistency_loss": [], "confidence_fraction": [], "effective_mask_rate": [], "cluster_scale": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        if epoch > args.warmup_epochs and ((epoch - args.warmup_epochs - 1) % max(1, args.target_update_interval) == 0 or p_targets is None):
            emb_for_target, p_targets = refresh_targets(model, full_loader, device)
            if not cluster_initialized:
                initialize_centers(model, emb_for_target, n_clusters, args.seed, device)
                emb_for_target, p_targets = refresh_targets(model, full_loader, device)
                cluster_initialized = True
        cluster_scale = 0.0 if epoch <= args.warmup_epochs else min(1.0, (epoch - args.warmup_epochs) / max(1, args.warmup_epochs))
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "cluster_loss", "consistency_loss", "confidence_fraction"]}
        mask_sum = 0.0
        n_batches = 0
        for idx, x_cpu, log_cpu, _ in train_loader:
            ...
            out = model(strong)
            weak_out = model(weak)
            p_batch = None if p_targets is None else torch.as_tensor(p_targets[idx.numpy()], dtype=torch.float32, device=device)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} cluster={history['cluster_loss'][-1]:.4f} conf={history['confidence_fraction'][-1]:.4f} scale={cluster_scale:.2f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + warmup cluster target"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, q_out, n_clusters, args.seed, preds)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_masked_sc_cluster_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_masked_sc_cluster_scmae.h5ad", compression="gzip")
