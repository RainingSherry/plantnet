...
from loss import MaskGITGeneTokenLoss
from model import MaskGITGeneTokenScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank23_maskgit_gene_token_full"
DISPLAY_NAME = "scMAE + MaskGIT gene-token curriculum"
...
class ExprDataset(Dataset):
    def __init__(self, encoder_data: np.ndarray, log_expr: np.ndarray, token_targets: np.ndarray, labels: np.ndarray):
        ...
        self.token_targets = torch.as_tensor(token_targets, dtype=torch.int16)
    ...
    def __getitem__(self, idx: int):
        return int(idx), self.encoder_data[idx], self.log_expr[idx], self.token_targets[idx], self.labels[idx]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent MaskGIT gene-token scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--mask_min", type=float, default=0.25)
    parser.add_argument("--mask_max", type=float, default=0.75)
    parser.add_argument("--replace_prob", type=float, default=0.30)
    parser.add_argument("--masked_data_weight", type=float, default=0.75)
    parser.add_argument("--mask_weight", type=float, default=0.55)
    parser.add_argument("--token_weight", type=float, default=0.16)
    parser.add_argument("--replaced_weight", type=float, default=0.08)
    parser.add_argument("--confidence_weight", type=float, default=0.01)
    return parser.parse_args()


def make_gene_tokens(log_expr: np.ndarray, token_bins: int) -> tuple[np.ndarray, np.ndarray]:
    x = np.asarray(log_expr, dtype=np.float32)
    edges = np.zeros((x.shape[1], max(0, token_bins - 1)), dtype=np.float32)
    tokens = np.zeros(x.shape, dtype=np.int16)
    qs = np.linspace(0.0, 1.0, token_bins + 1)[1:-1]
    for g in range(x.shape[1]):
        col = x[:, g]
        finite = col[np.isfinite(col)]
        positive = finite[finite > 1e-8]
        source = positive if positive.size >= token_bins else finite
        if source.size == 0:
            e = np.zeros(token_bins - 1, dtype=np.float32)
        else:
            e = np.quantile(source, qs).astype(np.float32)
            e = np.maximum.accumulate(e)
        edges[g] = e
        tokens[:, g] = np.clip(np.digitize(col, e, right=False), 0, token_bins - 1).astype(np.int16)
    return tokens, edges


def cosine_mask_ratio(mask_min: float, mask_max: float) -> float:
    progress = float(torch.rand(1).item())
    ratio01 = math.cos(0.5 * math.pi * progress)
    return float(mask_min + (mask_max - mask_min) * ratio01)


@torch.no_grad()
def extract_embedding(model: MaskGITGeneTokenScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, replaced_fraction: float, confidence: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "replaced_expression_fraction": float(replaced_fraction),
        "masked_token_confidence": float(confidence),
        "diagnostic_note": "No NeighborMix graph is used; MaskGIT-style cosine curriculum masked gene-token prediction is used.",
    }
...
def main() -> int:
    ...
    token_targets, token_edges = make_gene_tokens(log_expr, args.token_bins)
    save_json(target_bundle.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "token_source": "gene-specific log_expr quantile bins", "maskgit_corruption": "cosine-scheduled masked genes are zeroed or replaced by another cell for replaced-expression detection"}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_names.npy", target_bundle.gene_names.astype(str))
    np.save(save_dir / "token_edges.npy", token_edges)

    dataset = ExprDataset(encoder_data, log_expr, token_targets, labels)
    ...
    model = MaskGITGeneTokenScMAE(encoder_data.shape[1], args.patch_size, args.hidden_size, args.token_bins, args.n_layers, args.num_heads, args.dropout).to(device)
    criterion = MaskGITGeneTokenLoss(args.masked_data_weight, args.mask_weight, args.token_weight, args.replaced_weight, args.confidence_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "token_loss": [], "replaced_loss": [], "confidence_loss": [], "masked_token_confidence": [], "replaced_fraction": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "token_loss", "replaced_loss", "confidence_loss", "masked_token_confidence", "replaced_fraction"]}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, token_cpu, _ in train_loader:
            x = x_cpu.to(device)
            target = log_cpu.to(device)
            tokens = token_cpu.to(device).long()
            batch_mask_ratio = cosine_mask_ratio(args.mask_min, args.mask_max)
            corrupted, mask, replaced = model.maskgit_corrupt(x, batch_mask_ratio, args.replace_prob)
            out = model(corrupted)
            loss, parts = criterion(out, target, tokens, mask, replaced)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} token={history['token_loss'][-1]:.4f} conf={history['masked_token_confidence'][-1]:.4f} replaced={history['replaced_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args), "gene_names": target_bundle.gene_names.astype(str), "token_edges": token_edges}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + MaskGIT gene token curriculum target"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    replaced_fraction = float(np.mean(history["replaced_fraction"])) if history["replaced_fraction"] else 0.0
    confidence = float(np.mean(history["masked_token_confidence"])) if history["masked_token_confidence"] else 0.0
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, replaced_fraction, confidence)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_maskgit_gene_token_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_maskgit_gene_token_scmae.h5ad", compression="gzip")
