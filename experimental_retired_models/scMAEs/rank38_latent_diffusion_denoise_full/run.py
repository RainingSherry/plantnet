...
from loss import LatentDiffusionDenoiseLoss
from model import LatentDiffusionDenoiseScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank38_latent_diffusion_denoise_full"
DISPLAY_NAME = "scMAE + latent diffusion denoise"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent latent diffusion denoising scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--eps_weight", type=float, default=0.08)
    parser.add_argument("--denoise_weight", type=float, default=0.04)
    parser.add_argument("--variance_weight", type=float, default=0.01)
    return parser.parse_args()


@torch.no_grad()
def extract_embedding(model: LatentDiffusionDenoiseScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, eps_target_norm: float, eps_pred_norm: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "eps_target_norm": float(eps_target_norm),
        "eps_pred_norm": float(eps_pred_norm),
        "diagnostic_note": "No NeighborMix graph is used; latent DDPM denoising is used only as regularization, not for generated sample evaluation.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "diffusion_target": "latent DDPM epsilon prediction on scMAE latent; generated samples are not evaluated"}, str(save_dir / "preprocess_config.json"))
    ...
    model = LatentDiffusionDenoiseScMAE(encoder_data.shape[1], args.hidden_size, args.time_dim, args.dropout).to(device)
    criterion = LatentDiffusionDenoiseLoss(args.masked_data_weight, args.mask_weight, args.eps_weight, args.denoise_weight, args.variance_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    betas = torch.linspace(args.beta_start, args.beta_end, args.diffusion_steps, device=device)
    alphas = 1.0 - betas
    alpha_bar = torch.cumprod(alphas, dim=0)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "eps_loss": [], "denoise_loss": [], "variance_loss": [], "eps_target_norm": [], "eps_pred_norm": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "eps_loss", "denoise_loss", "variance_loss", "eps_target_norm", "eps_pred_norm"]}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            step = torch.randint(0, args.diffusion_steps, (z_clean.shape[0],), device=device)
            eps_target = torch.randn_like(z_clean)
            a_bar = alpha_bar[step].view(-1, 1)
            z_t = torch.sqrt(a_bar) * z_clean.detach() + torch.sqrt(1.0 - a_bar) * eps_target
            t_cont = step.float() / max(1, args.diffusion_steps - 1)
            eps_pred = model.predict_eps(z_t, t_cont)
            z0_hat = (z_t - torch.sqrt(1.0 - a_bar) * eps_pred) / torch.sqrt(a_bar).clamp_min(1e-4)
            loss, parts = criterion(out, target, mask, eps_pred, eps_target, z0_hat, z_clean)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} eps={history['eps_loss'][-1]:.4f} denoise={history['denoise_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + latent DDPM denoising regularizer"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, history["eps_target_norm"][-1], history["eps_pred_norm"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_latent_diffusion_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_latent_diffusion_scmae.h5ad", compression="gzip")
