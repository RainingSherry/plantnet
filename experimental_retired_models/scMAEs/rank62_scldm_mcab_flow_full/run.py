...
from loss import ScLdmMcabFlowLoss
from model import ScLdmMcabFlowScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank62_scldm_mcab_flow_full"
DISPLAY_NAME = "scMAE + scLDM MCAB flow"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent scLDM MCAB flow scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--flow_weight", type=float, default=0.035)
    p.add_argument("--norm_weight", type=float, default=0.005)
...
@torch.no_grad()
def extract_embedding(model: ScLdmMcabFlowScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, flow_t_mean: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "flow_timestep_mean": float(flow_t_mean),
        "input_mask_rate": float(mask_rate),
        "diagnostic_note": "No NeighborMix is used. scLDM is adapted as MCAB-style gene-token latent pooling plus flow matching only.",
    }
...
def main() -> int:
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "rank62_core": "MCAB-style gene-token pooling plus linear-interpolant latent flow"}, str(save_dir / "preprocess_config.json"))
    dataset = McabFlowDataset(encoder_data, log_expr, labels)
    ...
    model = ScLdmMcabFlowScMAE(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, args.token_dim, args.n_latents, args.n_heads, args.time_dim, args.dropout).to(device)
    criterion = ScLdmMcabFlowLoss(args.masked_data_weight, args.mask_loss_weight, args.flow_weight, args.norm_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "flow_loss", "norm_loss", "variance_loss", "effective_mask_rate", "flow_timestep_mean"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            z_t = (1.0 - t[:, None]) * noise + t[:, None] * z
            flow_target = z - noise
            flow_pred = model.predict_flow(z_t, t)
            loss, parts = criterion(out, y, mask, flow_pred, flow_target)
            ...
            sums["flow_timestep_mean"] += float(t.mean().detach().cpu())
            nb += 1
        for k in keys:
            history[k].append(sums[k] / max(1, nb))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} flow={history['flow_loss'][-1]:.4f} norm={history['norm_loss'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + MCAB flow"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], history["flow_timestep_mean"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_scldm_mcab_flow_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_scldm_mcab_flow_scmae.h5ad", compression="gzip")
