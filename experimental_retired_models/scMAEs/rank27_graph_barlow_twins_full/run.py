...
from loss import GraphBarlowTwinsLoss
...
METHOD_NAME = "rank27_graph_barlow_twins_full"
DISPLAY_NAME = "scMAE + Graph Barlow Twins"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent Graph Barlow Twins scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--barlow_weight", type=float, default=0.04)
...
@torch.no_grad()
def extract_embedding(model: GraphBarlowTwinsScMAE, loader: DataLoader, all_x: torch.Tensor, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, edge_conf: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, edge_survival: float) -> dict:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix is used. Graph Barlow Twins supplies symmetric graph view invariance and redundancy reduction around the scMAE objective.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "graph_knn": f"TruncatedSVD({args.knn_pca_dim}) + cosine KNN(k={args.neighbor_k})", "graph_barlow_target": "symmetric feature masking and DropEdge views with cross-correlation redundancy reduction"}, str(save_dir / "preprocess_config.json"))
    ...
    model = GraphBarlowTwinsScMAE(encoder_data.shape[1], args.hidden_size, args.dropout).to(device)
    criterion = GraphBarlowTwinsLoss(args.masked_data_weight, args.mask_weight, args.barlow_weight, args.edge_weight, args.neighbor_weight, args.variance_weight)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {k: [] for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "barlow_loss", "barlow_on_diag", "barlow_off_diag", "edge_loss", "neighbor_loss", "variance_loss", "edge_confidence", "edge_negative_confidence", "edge_proxy_accuracy", "edge_survival", "effective_mask_rate"]}
    history["stage"] = stage
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _, neigh_ids in train_loader:
            ...
            z_view1 = model.encode(x1, n1)
            z_view2 = model.encode(x2, n2)
            ...
            loss, parts = criterion(out, target, mask, z_view1, z_view2, edge_logits, neighbor_latent)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} barlow={history['barlow_loss'][-1]:.4f} edge={history['edge_loss'][-1]:.4f} edge_conf={history['edge_confidence'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + shallow KNN Graph Barlow Twins"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_graph_barlow_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_graph_barlow_scmae.h5ad", compression="gzip")
