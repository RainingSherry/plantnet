...
from loss import BGRLGraphBootstrapLoss
from model import BGRLGraphScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank26_bgrl_graph_bootstrap_full"
DISPLAY_NAME = "scMAE + BGRL graph bootstrap"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent BGRL graph-bootstrap scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--feat_drop2", type=float, default=0.35)
    parser.add_argument("--edge_drop1", type=float, default=0.15)
    parser.add_argument("--edge_drop2", type=float, default=0.25)
    parser.add_argument("--ema_base", type=float, default=0.99)
    parser.add_argument("--masked_data_weight", type=float, default=0.75)
    parser.add_argument("--mask_weight", type=float, default=0.60)
    parser.add_argument("--bootstrap_weight", type=float, default=0.08)
    parser.add_argument("--edge_weight", type=float, default=0.04)
    parser.add_argument("--neighbor_weight", type=float, default=0.03)
...
@torch.no_grad()
def extract_embedding(model: BGRLGraphScMAE, loader: DataLoader, all_x: torch.Tensor, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
    ...
    for _, x_cpu, _, y, neigh_ids in loader:
        ...
        z_pos = model.encode_online(pos_x, None)
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, edge_conf: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, edge_survival: float) -> dict:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix is used. BGRL supplies shallow graph bootstrap, edge confidence, and DropEdge robustness around the scMAE objective.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "graph_knn": f"TruncatedSVD({args.knn_pca_dim}) + cosine KNN(k={args.neighbor_k})", "bgrl_target": "EMA target encoder with feature masking and DropEdge views"}, str(save_dir / "preprocess_config.json"))
    ...
    model = BGRLGraphScMAE(encoder_data.shape[1], args.hidden_size, args.predictor_hidden, args.dropout).to(device)
    criterion = BGRLGraphBootstrapLoss(args.masked_data_weight, args.mask_weight, args.bootstrap_weight, args.edge_weight, args.neighbor_weight, args.variance_weight)
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    total_steps = max(1, args.epochs * len(train_loader))
    history = {k: [] for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "bootstrap_loss", "edge_loss", "neighbor_loss", "variance_loss", "edge_confidence", "edge_negative_confidence", "edge_proxy_accuracy", "ema_momentum", "edge_survival", "effective_mask_rate"]}
    history["stage"] = stage
    start = time.time()
    global_step = 0
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        n_batches = 0
        for _, x_cpu, log_cpu, _, neigh_ids in train_loader:
            x = x_cpu.to(device)
            target = log_cpu.to(device)
            neigh1, surv1 = gather_neighbors(all_encoder, neigh_ids, device, args.edge_drop1)
            neigh2, surv2 = gather_neighbors(all_encoder, neigh_ids, device, args.edge_drop2)
            corrupted, mask = model.mask_view(x, args.mask_prob)
            out = model(corrupted, neigh1)
            x1 = model.feature_drop(x, args.feat_drop1)
            x2 = model.feature_drop(x, args.feat_drop2)
            n1 = model.feature_drop(neigh1, args.feat_drop1)
            n2 = model.feature_drop(neigh2, args.feat_drop2)
            pred12, target12 = model.bootstrap(x1, n1, x2, n2)
            pred21, target21 = model.bootstrap(x2, n2, x1, n1)
            ...
            pos_z = model.encode_online(pos_x, None)
            neg_z = model.encode_online(neg_x, None)
            edge_logits = model.edge_logits(out["latent"], pos_z, neg_z)
            with torch.no_grad():
                neighbor_latent = model.encode_target(pos_x, None)
            loss, parts = criterion(out, target, mask, [(pred12, target12), (pred21, target21)], edge_logits, neighbor_latent)
            ...
            optimizer.step()
            momentum = model.cosine_momentum(args.ema_base, global_step, total_steps)
            model.update_target(momentum)
            global_step += 1
            for key, value in parts.items():
                sums[key] += value
            sums["ema_momentum"] += momentum
            sums["edge_survival"] += 0.5 * (surv1 + surv2)
            sums["effective_mask_rate"] += float(mask.mean().detach().cpu())
            n_batches += 1
        for key in sums:
            history[key].append(sums[key] / max(1, n_batches))
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} bgrl={history['bootstrap_loss'][-1]:.4f} edge={history['edge_loss'][-1]:.4f} edge_conf={history['edge_confidence'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + shallow KNN graph bootstrap"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_bgrl_graph_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_bgrl_graph_scmae.h5ad", compression="gzip")
