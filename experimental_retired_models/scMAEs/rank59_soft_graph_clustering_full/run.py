...
from loss import SoftGraphLoss
from model import SoftGraphScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank59_soft_graph_clustering_full"
DISPLAY_NAME = "scMAE + soft graph clustering"
...
class SoftGraphDataset(Dataset):
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent soft graph clustering scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--graph_k", type=int, default=10)
    ...
    p.add_argument("--graph_weight", type=float, default=0.025)
...
def corrupt(x: torch.Tensor, mask_prob: float, replace_prob: float) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if replace_prob >= 1.0:
        return torch.where(mask.bool(), x[torch.randperm(x.shape[0], device=x.device)], x), mask
...
def make_soft_graph(log_expr: np.ndarray, k: int, svd_dim: int, seed: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    nn_e = NearestNeighbors(n_neighbors=n_neighbors, metric="euclidean").fit(rep)
    dist, idx = nn_e.kneighbors(rep)
    idx = idx[:, 1:]
    dist = dist[:, 1:]
    eu = np.exp(-dist / (np.median(dist) + 1e-6))
    rep_norm = rep / (np.linalg.norm(rep, axis=1, keepdims=True) + 1e-6)
    cos = np.maximum(0.0, (rep_norm[:, None, :] * rep_norm[idx]).sum(axis=2))
    weight = 0.5 * eu + 0.5 * cos
    weight = np.clip(weight, 0.02, 0.98).astype(np.float32)
    return idx.astype(np.int64), weight


@torch.no_grad()
def extract_embedding(model: SoftGraphScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, edge_confidence: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, edge_survival: float) -> dict:
    if preds is None:
    ...
    return {
        ...
        "edge_confidence_mean": float(np.mean(edge_confidence)) if edge_confidence.size else float("nan"),
        "edge_confidence_std": float(np.std(edge_confidence)) if edge_confidence.size else float("nan"),
        "input_mask_rate": float(mask_rate),
        "diagnostic_note": "No NeighborMix is used. Soft graph is used as shallow edge-confidence and residual consistency regularization only.",
    }
...
def main() -> int:
    ...
    if args.smoke:
        ...
        args.graph_k = min(args.graph_k, 8)
    ...
    neighbor_idx, edge_weight = make_soft_graph(log_expr, args.graph_k, args.svd_dim, args.seed)
    np.save(save_dir / "gene_names.npy", target.gene_names.astype(str))
    np.save(save_dir / "soft_graph_neighbors.npy", neighbor_idx.astype(np.int64))
    np.save(save_dir / "soft_graph_weights.npy", edge_weight.astype(np.float32))
    save_json(target.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "rank59_core": "dual-channel soft graph edge confidence and residual consistency regularization"}, str(save_dir / "preprocess_config.json"))
    dataset = SoftGraphDataset(encoder_data, log_expr, labels)
    ...
    model = SoftGraphScMAE(encoder_data.shape[1], args.hidden_size, args.decoder_hidden, args.dropout).to(device)
    criterion = SoftGraphLoss(args.masked_data_weight, args.mask_loss_weight, args.graph_weight, args.edge_weight, args.variance_weight)
    ...
    graph_w = torch.as_tensor(edge_weight, dtype=torch.float32, device=device)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "graph_loss", "edge_loss", "variance_loss", "effective_mask_rate", "edge_confidence_mean", "edge_survival"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for idx_cpu, x_cpu, log_cpu, _ in train_loader:
            ...
            out = model(corrupted)
            neigh_idx = graph_idx[idx]
            neigh_z = memory[neigh_idx]
            target_w = graph_w[idx]
            edge_keep = (torch.rand_like(target_w) > args.dropedge).float()
            loss, parts, _ = criterion(model, out, y, mask, neigh_z, target_w, edge_keep)
            ...
            opt.step()
            with torch.no_grad():
                memory[idx] = args.memory_momentum * memory[idx] + (1.0 - args.memory_momentum) * out["embedding"].detach()
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} graph={history['graph_loss'][-1]:.4f} edge={history['edge_loss'][-1]:.4f} edge_surv={history['edge_survival'][-1]:.3f}")
    embedding, labels_out, mask_scores = extract_embedding(model, full_loader, device)
    with torch.no_grad():
        edge_conf = []
        emb_t = torch.as_tensor(embedding, dtype=torch.float32, device=device)
        for start_i in range(0, embedding.shape[0], max(args.batch_size * 4, 512)):
            end_i = min(embedding.shape[0], start_i + max(args.batch_size * 4, 512))
            z = emb_t[start_i:end_i]
            neigh = emb_t[graph_idx[start_i:end_i]]
            edge_conf.append(torch.sigmoid(model.edge_logits(z, neigh)).detach().cpu().numpy())
        edge_confidence = np.concatenate(edge_conf, axis=0).astype(np.float32)
    ...
    np.save(save_dir / "edge_confidence.npy", edge_confidence.astype(np.float32))
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + soft graph regularization"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, edge_confidence, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], history["edge_survival"][-1])
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_soft_graph_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_soft_graph_scmae.h5ad", compression="gzip")
