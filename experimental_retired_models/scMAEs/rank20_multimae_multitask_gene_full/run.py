...
from loss import MultiMAEGeneLoss
from model import MultiMAEGeneScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank20_multimae_multitask_gene_full"
DISPLAY_NAME = "scMAE + MultiMAE gene multitask"
...
class ExprDataset(Dataset):
    def __init__(self, encoder_data: np.ndarray, log_expr: np.ndarray, token_targets: np.ndarray, module_targets: np.ndarray, labels: np.ndarray):
    ...
    def __getitem__(self, idx: int):
        return int(idx), self.encoder_data[idx], self.log_expr[idx], self.token_targets[idx], self.module_targets[idx], self.labels[idx]


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent MultiMAE gene multitask scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def make_gene_tokens(log_expr: np.ndarray, token_bins: int) -> tuple[np.ndarray, np.ndarray]:
    ...
    for g in range(x.shape[1]):
        ...
        e = np.zeros(token_bins - 1, dtype=np.float32) if source.size == 0 else np.maximum.accumulate(np.quantile(source, qs).astype(np.float32))
        edges[g] = e
        tokens[:, g] = np.clip(np.digitize(col, e, right=False), 0, token_bins - 1).astype(np.int16)
    return tokens, edges


def module_targets(log_expr: np.ndarray, module_size: int) -> np.ndarray:
    n_modules = (log_expr.shape[1] + module_size - 1) // module_size
    padded = np.pad(log_expr, ((0, 0), (0, n_modules * module_size - log_expr.shape[1])), mode="constant")
    return padded.reshape(log_expr.shape[0], n_modules, module_size).mean(axis=2).astype(np.float32)


def batch_module_mask(mask: torch.Tensor, module_size: int, n_modules: int) -> torch.Tensor:
    padded = torch.nn.functional.pad(mask, (0, n_modules * module_size - mask.shape[1]))
    return (padded.view(mask.shape[0], n_modules, module_size).mean(dim=2) > 0).float()


@torch.no_grad()
def extract_embedding(model: MultiMAEGeneScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None) -> dict:
    if preds is None:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix graph is used; MultiMAE-style expression/token/module decoders are used.",
    }
...
def main() -> int:
    ...
    modules = module_targets(log_expr, args.module_size)
    save_json(target_bundle.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "targets": "log_expr reconstruction + gene-specific token + module summary"}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_names.npy", target_bundle.gene_names.astype(str))
    np.save(save_dir / "token_edges.npy", token_edges)

    dataset = ExprDataset(encoder_data, log_expr, token_targets, modules, labels)
    ...
    model = MultiMAEGeneScMAE(encoder_data.shape[1], args.hidden_size, args.token_bins, args.module_size, args.dropout).to(device)
    criterion = MultiMAEGeneLoss(args.masked_data_weight, args.mask_weight, args.token_weight, args.module_weight)
    optimizer = torch.optim.Adam(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    history = {"loss": [], "scmae_loss": [], "reconstruction_loss": [], "mask_loss": [], "token_loss": [], "module_loss": [], "effective_mask_rate": [], "stage": stage}
    start = time.time()
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        model.train()
        sums = {k: 0.0 for k in ["loss", "scmae_loss", "reconstruction_loss", "mask_loss", "token_loss", "module_loss"]}
        mask_sum = 0.0
        n_batches = 0
        for _, x_cpu, log_cpu, token_cpu, module_cpu, _ in train_loader:
            x = x_cpu.to(device)
            target = log_cpu.to(device)
            tokens = token_cpu.to(device).long()
            module_target = module_cpu.to(device)
            corrupted, mask = model.multitask_mask(x, args.mask_prob)
            module_mask = batch_module_mask(mask, args.module_size, model.n_modules)
            out = model(corrupted)
            loss, parts = criterion(out, target, tokens, module_target, mask, module_mask)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} token={history['token_loss'][-1]:.4f} module={history['module_loss'][-1]:.4f} mask_rate={history['effective_mask_rate'][-1]:.4f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + MultiMAE expression/token/module targets"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_multimae_gene_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_multimae_gene_scmae.h5ad", compression="gzip")
