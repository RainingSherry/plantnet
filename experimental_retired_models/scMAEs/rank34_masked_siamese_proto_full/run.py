...
from loss import MaskedSiameseProtoLoss
from model import MaskedSiameseProtoScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank34_masked_siamese_proto_full"
DISPLAY_NAME = "scMAE + Masked Siamese prototypes"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent Masked Siamese Network prototype scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--proto_weight", type=float, default=0.12)
    parser.add_argument("--memax_weight", type=float, default=0.04)
    ...
    parser.add_argument("--sharpen_temperature", type=float, default=0.25)
...
@torch.no_grad()
def extract_embedding(model: MaskedSiameseProtoScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
def diagnostics(
    ...
    target_conf = float(np.mean(history.get("target_confidence", [float("nan")])))
    used_proto = float(np.mean(history.get("target_used_prototypes", [float("nan")])))
    return {
        "edge_survival": 1.0,
        ...
        "target_confidence_mean": target_conf,
        "target_used_prototypes_mean": used_proto,
        "diagnostic_note": "No NeighborMix is used. MSN matches masked-anchor representations to EMA-teacher prototype distributions.",
    }
...
def main() -> int:
    ...
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "msn_target": "EMA teacher sharpened prototype distribution for the unmasked/weak target view", "neighbor_mix": "not used"}, str(save_dir / "preprocess_config.json"))
    ...
    criterion = MaskedSiameseProtoLoss(
        ...
        args.temperature,
        args.sharpen_temperature,
    ).to(device)
    optimizer = torch.optim.AdamW(
        [{"params": model.student.parameters()}, {"params": model.mask_predictor.parameters()}, {"params": model.decoder.parameters()}, {"params": [model.prototypes]}],
        lr=args.lr,
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            proto_scale = min(1.0, float(epoch) / max(1.0, float(args.warmup_epochs)))
            loss, parts = criterion(model, student_out, teacher_out, target, gene_mask, proto_scale)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} proto={history['proto_ce_loss'][-1]:.4f} memax={history['memax_loss'][-1]:.4f} m={history['ema_momentum'][-1]:.5f}")
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + MSN prototype target"})
    ...
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_masked_siamese_proto_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_masked_siamese_proto_scmae.h5ad", compression="gzip")
