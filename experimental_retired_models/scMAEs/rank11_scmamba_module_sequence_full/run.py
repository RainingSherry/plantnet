...
from loss import ScMambaModuleLoss
from model import ScMambaModuleScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json


METHOD_NAME = "rank11_scmamba_module_sequence_full"
DISPLAY_NAME = "scMAE + scMamba module sequence"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Independent scMamba module sequence scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def coexpression_gene_order(log_expr: np.ndarray, n_anchors: int = 8, max_cells: int = 5000, seed: int = 42) -> np.ndarray:
...
def module_targets(ordered_log: np.ndarray, patch_size: int) -> np.ndarray:
    ...
    return padded.reshape(ordered_log.shape[0], n_patches, patch_size).mean(axis=2).astype(np.float32)


@torch.no_grad()
def extract_embedding(model: ScMambaModuleScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray]:
    model.eval()
    emb, labels = [], []
...
def diagnostics(embedding: np.ndarray, labels: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, module_var: float) -> dict:
    ...
    return {
        ...
        "diagnostic_note": "No NeighborMix graph is used; genes are coexpression-ordered and encoded as module patches.",
    }
...
def main() -> int:
    ...
    ordered_log = log_expr[:, gene_order]
    modules = module_targets(ordered_log, args.patch_size)
    save_json(target_bundle.profile, str(save_dir / "dataset_profile.json"))
    save_json({**target_bundle.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "gene_order_source": "coexpression anchor order from log_expr", "module_target_source": "mean log_expr per ordered patch"}, str(save_dir / "preprocess_config.json"))
    np.save(save_dir / "gene_names.npy", target_bundle.gene_names.astype(str))
    np.save(save_dir / "gene_order.npy", gene_order)

    dataset = ExprDataset(ordered_encoder, ordered_log, modules, labels)
    ...
    model = ScMambaModuleScMAE(ordered_encoder.shape[1], args.patch_size, args.hidden_size, args.n_layers, args.dropout).to(device)
    criterion = ScMambaModuleLoss(args.masked_data_weight, args.mask_weight, args.module_weight, args.consistency_weight)
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        ...
        for _, x_cpu, log_cpu, module_cpu, _ in train_loader:
            ...
            strong, mask = model.block_mask(x, args.mask_prob)
            weak, _ = model.block_mask(x, max(0.05, args.mask_prob * 0.5))
            out = model(strong)
            weak_out = model(weak)
            loss, parts = criterion(out, weak_out, target, module_target, mask)
    ...
    torch.save({"model": model.state_dict(), "optimizer": optimizer.state_dict(), "args": vars(args), "gene_order": gene_order, "gene_names": target_bundle.gene_names.astype(str)}, save_dir / "model_checkpoint.pth")

    eval_result, preds = None, None
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "coexpression ordered module sequence"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, n_clusters, args.seed, preds, float(np.var(modules)))
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        encoder_bundle.adata.obsm["X_scmamba_module_scmae"] = embedding
        ...
        encoder_bundle.adata.write_h5ad(save_dir / "adata_scmamba_module_scmae.h5ad", compression="gzip")
