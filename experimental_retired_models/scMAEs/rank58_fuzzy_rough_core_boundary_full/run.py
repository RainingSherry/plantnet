...
from loss import FuzzyRoughLoss
from model import FuzzyRoughScMAE
from methods.DeepLearning import scMAE_family as family
from methods.shared_utils import ensure_dir, sanitize_anndata_for_write, save_json

METHOD_NAME = "rank58_fuzzy_rough_core_boundary_full"
DISPLAY_NAME = "scMAE + fuzzy-rough core boundary"
...
class FuzzyRoughDataset(Dataset):
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Independent fuzzy-rough core-boundary scMAE candidate.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    p.add_argument("--fuzzy_start_epoch", type=int, default=10)
    p.add_argument("--core_threshold", type=float, default=0.55)
    p.add_argument("--masked_data_weight", type=float, default=0.75)
    p.add_argument("--mask_loss_weight", type=float, default=0.7)
    p.add_argument("--core_weight", type=float, default=0.04)
    p.add_argument("--balance_weight", type=float, default=0.01)
    p.add_argument("--separation_weight", type=float, default=0.002)
...
@torch.no_grad()
def extract_embedding(model: FuzzyRoughScMAE, loader: DataLoader, device: torch.device) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    model.eval()
...
@torch.no_grad()
def initialize_centers(model: FuzzyRoughScMAE, loader: DataLoader, device: torch.device, n_clusters: int, seed: int) -> None:
    emb, _, _ = extract_embedding(model, loader, device)
    centers = KMeans(n_clusters=n_clusters, n_init=20, random_state=seed).fit(emb).cluster_centers_.astype(np.float32)
    model.set_centers(torch.as_tensor(centers, device=device))


def diagnostics(embedding: np.ndarray, labels: np.ndarray, mask_scores: np.ndarray, membership: np.ndarray, n_clusters: int, seed: int, preds: np.ndarray | None, mask_rate: float, core_threshold: float) -> dict:
    if preds is None:
    ...
    mem = np.clip(membership, 1e-8, 1.0)
    mem_entropy = float(np.mean(-(mem * np.log(mem)).sum(axis=1) / max(np.log(max(2, mem.shape[1])), 1e-8))) if mem.size else float("nan")
    core_fraction = float(np.mean(mem.max(axis=1) >= core_threshold)) if mem.size else 0.0
    return {
        ...
        "fuzzy_membership_entropy": mem_entropy,
        "fuzzy_core_fraction": core_fraction,
        "diagnostic_note": "No NeighborMix is used. Fuzzy-rough adaptation reinforces high-confidence core cells and leaves boundary cells weakly constrained.",
    }
...
def main() -> int:
    ...
    if args.smoke:
        ...
        args.fuzzy_start_epoch = min(args.fuzzy_start_epoch, 2)
    ...
    save_json({**target.preprocess_config, "encoder_scale_input": bool(args.scale_input), "target_scale_input": False, "rank58_core": "fuzzy-rough lower/upper approximation mapped to core/boundary latent clustering"}, str(save_dir / "preprocess_config.json"))
    dataset = FuzzyRoughDataset(encoder_data, log_expr, labels)
    ...
    model = FuzzyRoughScMAE(encoder_data.shape[1], n_clusters, args.hidden_size, args.decoder_hidden, args.dropout).to(device)
    criterion = FuzzyRoughLoss(args.masked_data_weight, args.mask_loss_weight, args.core_weight, args.balance_weight, args.separation_weight, args.variance_weight)
    opt = torch.optim.AdamW(model.parameters(), lr=args.lr, weight_decay=args.weight_decay)
    keys = ["loss", "scmae_loss", "recon_loss", "mask_loss", "core_loss", "balance_loss", "separation_loss", "variance_loss", "effective_mask_rate", "core_fraction", "membership_entropy"]
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        fuzzy_active = epoch >= args.fuzzy_start_epoch
        if fuzzy_active and not centers_initialized:
            initialize_centers(model, full_loader, device, n_clusters, args.seed)
        ...
        for _, x_cpu, log_cpu, _ in train_loader:
            ...
            out = model(corrupted)
            loss, parts, _ = criterion(model, out, y, mask, fuzzy_active, args.core_threshold)
        ...
        if epoch == 1 or epoch == args.epochs or epoch % 10 == 0:
            print(f"Epoch {epoch:03d}/{args.epochs} loss={history['loss'][-1]:.4f} scmae={history['scmae_loss'][-1]:.4f} core={history['core_loss'][-1]:.4f} core_frac={history['core_fraction'][-1]:.3f} mask={history['effective_mask_rate'][-1]:.3f}")

    embedding, labels_out, mask_scores = extract_embedding(model, full_loader, device)
    with torch.no_grad():
        membership_batches = []
        model.eval()
        for _, x_cpu, _, _ in full_loader:
            membership_batches.append(model(x_cpu.to(device))["membership"].detach().cpu().numpy())
        membership = np.concatenate(membership_batches).astype(np.float32)
    ...
    np.save(save_dir / "fuzzy_membership.npy", membership.astype(np.float32))
    ...
    if not args.skip_eval:
        eval_result = family.write_kmeans_known_k_outputs(save_dir, dataset_name, DISPLAY_NAME, args.seed, embedding, labels_out, n_clusters, {"variant": METHOD_NAME, "stage": stage, "preprocessing": "scaled encoder + log-expression targets + fuzzy-rough core-boundary"})
        preds = eval_result["preds"]["kmeans_known_k"]
        save_json(eval_result["fixed"], str(save_dir / "metrics.json"))
    diag = diagnostics(embedding, labels_out, mask_scores, membership, n_clusters, args.seed, preds, history["effective_mask_rate"][-1], args.core_threshold)
    save_json(diag, str(save_dir / "diagnostics.json"))
    if not args.no_save_h5ad:
        enc.adata.obsm["X_fuzzy_rough_scmae"] = embedding
        enc.adata.uns[METHOD_NAME] = {"method": DISPLAY_NAME, "variant": METHOD_NAME, "stage": stage}
        sanitize_anndata_for_write(enc.adata)
        enc.adata.write_h5ad(save_dir / "adata_fuzzy_rough_scmae.h5ad", compression="gzip")
