...
class QDiffusionScMAE(nn.Module):
    """scMAE backbone with a lightweight q-diffusion latent denoiser."""

    def __init__(self, input_dim: int, hidden_size: int = 128, decoder_hidden: int = 128, time_dim: int = 64, dropout: float = 0.0):
        super().__init__()
        ...
        self.denoiser = nn.Sequential(
            nn.Linear(hidden_size * 2 + time_dim, decoder_hidden),
    ...
    def denoise_latent(self, noisy_z: torch.Tensor, t: torch.Tensor, q_context: torch.Tensor) -> torch.Tensor:
        time = self.time_embedding(t)
        return self.denoiser(torch.cat([noisy_z, q_context, time], dim=1))
