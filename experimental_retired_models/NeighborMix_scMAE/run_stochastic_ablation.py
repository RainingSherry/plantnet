...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="NeighborMix stochastic regularization and neighbor-backend ablation")
...
def build_graph_for_method(args: argparse.Namespace, data_np: np.ndarray) -> NeighborGraph:
    ...
    if method == "mutual_knn_neighbormix":
        return build_mutual_graph(data_np, args.neighbor_k, args.knn_pca_dim, args.tau, args.seed)
    if method == "snn_neighbormix":
        return build_snn_graph(data_np, args.neighbor_k, args.knn_pca_dim, args.tau, args.seed)
    if method == "consensus_neighbormix_threshold0.4":
        return build_consensus_graph(data_np, args.neighbor_k, args.knn_pca_dim, args.tau, args.seed, args.consensus_threshold)
    if method == "global_random_neighbor_control":
        return build_global_random_graph(data_np, args.neighbor_k, args.knn_pca_dim, args.tau, args.seed)
    return build_vanilla_graph(data_np, args.neighbor_k, args.knn_pca_dim, args.tau, args.seed)


def method_config(method: str, args: argparse.Namespace) -> dict:
    beta = 1.0 - float(args.alpha)
    cfg = {
...
def main() -> int:
    ...
    graph = build_graph_for_method(args, data_np)
    ...
    for epoch in range(1, max(1, int(args.epochs)) + 1):
        ...
        for idx_t, x_cpu, _ in train_loader:
            ...
            if pseudo_enabled:
                ...
                if cfg["beta_mode"] == "uniform":
                    beta_np = rng.uniform(0.0, float(cfg["beta_max"]), size=idx_np.shape[0]).astype(np.float32)
                else:
                    beta_np = np.full(idx_np.shape[0], float(cfg["beta_fixed"]), dtype=np.float32)
    ...
    summary = {
        ...
        "label_leakage": False,
        "interpretation_note": "Pseudo branch reconstructs anchor cells; stochastic gates are regularization tests, not reliable-cell selection.",
    }
