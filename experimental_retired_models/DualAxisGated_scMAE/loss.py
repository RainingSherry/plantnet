...
class GatedFusionLoss(nn.Module):
    ...
    def _scmae_percell(self, out: dict, target: torch.Tensor, mask: torch.Tensor, gene_weight: torch.Tensor | None = None) -> torch.Tensor:
        weights = mask * self.masked_data_weight + (1.0 - mask) * (1.0 - self.masked_data_weight)
        if gene_weight is not None:
            weights = weights * gene_weight.view(1, -1)      # marker genes weigh more in reconstruction
    ...
    def forward(
        ...
        cluster_scale: float,
        gene_weight: torch.Tensor | None = None,
    ) -> tuple[torch.Tensor, dict]:
        # rank13 scmae (mean over cells) — kept exactly
        scmae = self._scmae_percell(out, target, mask, gene_weight).mean()
        ...
        if pseudo_out is not None and pseudo_mask is not None and pseudo_weight > 0:
            per_cell = self._scmae_percell(pseudo_out, target, pseudo_mask, gene_weight)
