...
PROJECT_ROOT = next(parent for parent in [CURRENT_DIR, *CURRENT_DIR.parents] if (parent / "methods" / "DeepLearning" / "scMAE_family.py").exists())
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))

from experimental_retired_models.APA_scMAE.config import build_arg_parser, config_hash, resolve_config, write_yaml
from experimental_retired_models.APA_scMAE.data import APAExpressionDataset, load_apa_data, save_text_lines
from experimental_retired_models.APA_scMAE.model import APAModel
from experimental_retired_models.APA_scMAE.trainer import APATrainer, save_json
