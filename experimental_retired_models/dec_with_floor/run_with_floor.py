...
def _register_null_h5ad_reader():
    """Register null reader to handle legacy h5ad encoding issues."""
    try:
        import h5py
...
# Add methods/ root to path for imports
...
def variance_floor_loss(z, floor_scale=1.0):
    """VICReg-style per-dimension variance floor: relu(floor_scale - std_d).mean()
    Prevents per-dimension variance collapse (the failure mode of DEC on fine-grained data).
    """
    std = torch.sqrt(z.var(dim=0) + 1e-4)
    return F.relu(floor_scale - std).mean()
...
def parse_args():
    ...
    parser.add_argument('--no_cuda', action='store_true',
                        help='Disable CUDA')
    parser.add_argument('--floor_weight', type=float, default=0.0,
                        help='Weight for variance floor loss (0=baseline DEC, 0.02=with floor)')
...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        q, z = model(X_tensor)
        kl_loss = F.kl_div(q.log(), p_tensor, reduction='batchmean')

        # Add variance floor loss (prevents per-dimension collapse)
        if args.floor_weight > 0:
            floor_loss = variance_floor_loss(z, floor_scale=1.0)
            loss = kl_loss + args.floor_weight * floor_loss
        else:
            loss = kl_loss
        ...
        if (epoch + 1) % 50 == 0:
            if args.floor_weight > 0:
                print(f'Epoch {epoch + 1}/{args.epochs}, KL: {kl_loss.item():.6f}, Floor: {floor_loss.item():.6f}, Total: {loss.item():.6f}')
            else:
                print(f'Epoch {epoch + 1}/{args.epochs}, Loss: {loss.item():.6f}')

    # Save results with full output files
