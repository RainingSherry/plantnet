#!/usr/bin/env python3
"""Checkpoint-based inference for cursor_Doloris_GraphDiffusion using strict=False loading."""
            ...
            try:
                ...
                if isinstance(ds, h5py.Group):
                    if 'codes' in ds and 'categories' in ds:
                        ...
                        obs[key] = np.array([s.decode() if isinstance(s, bytes) else s for s in cats])[codes]
                    else: continue
                else:
                    vals = np.array(ds[:])
                    obs[key] = np.array([s.decode() if isinstance(s, bytes) else s for s in vals]) if vals.dtype.kind == 'O' else vals
            except: continue
    var_names = None
    if 'var' in f:
        for vk in ['_index', 'gene_name']:
            if vk in f['var']:
                vals = np.array(f[f'var/{vk}'][:])
                var_names = np.array([s.decode() if isinstance(s, bytes) else s for s in vals])
                break
    f.close()
...
class SimpleDataset(torch.utils.data.Dataset):
    def __init__(self, X_hvg, X_raw, hvg_idx, labels, topk_support=2000):
        self.X = X_hvg; self.X_raw = X_raw; self.hvg_idx = hvg_idx; self.labels = labels
        self.n_cells = X_hvg.shape[0]; self.n_hvg = X_hvg.shape[1]
        support_idx_list = []; support_weight_list = []; max_support = 0
        for c in range(self.n_cells):
            expr = np.clip(X_raw[c, hvg_idx], 0, None)
            k = min(topk_support, len(hvg_idx))
            topk_local = np.argpartition(expr, -k)[-k:]
            topk_local = topk_local[np.argsort(expr[topk_local])[::-1]]
            support_idx_list.append(topk_local.astype(np.int64))
            vals = expr[topk_local]; w_sum = vals.sum() + 1e-8
            support_weight_list.append((vals / w_sum).astype(np.float32))
            max_support = max(max_support, len(topk_local))
        self.max_support = min(((max_support + 31) // 32) * 32, 2000)
...
def build_graph(X_hvg, top_k=20):
    ...
    weight_list = [0.5 + 0.5 * (w - w_min) / (w_max - w_min) for w in weight_list] if w_max > w_min else [1.0]*len(weight_list)
    return {"edge_index": [row_list, col_list], "edge_weight": weight_list, "n_nodes": n_genes, "graph_type": "coexpression"}


def run(data_path, result_dir, config_path, checkpoint_path, n_clusters_arg, device):
    rd = Path(result_dir); rd.mkdir(parents=True, exist_ok=True)
    mf = rd / "metrics.json"
    if mf.exists():
        print(f"metrics.json exists, skip: {mf}")
        return
    ...
    valid = y_true != "Unknow"
    X_raw_v = X_raw[valid]; y_true_v = y_true[valid]
    n_ct_actual = len(np.unique(y_true_v))
    print(f"  {X_raw_v.shape[0]} cells, {n_ct_actual} types")

    X_log = np.log1p(X_raw_v)
    ...
    graph_dict = build_graph(X_hvg, top_k=20)
    ...
    # Use config's n_clusters (15 for SRP182008, 13 for Mouse_Pancreas_1)
    n_actual_clusters = sc.get("n_clusters", n_clusters_arg)
    cell_type_num = sc.get("cell_type_num", n_ct_actual)
    print(f"  Using n_clusters={n_actual_clusters}, cell_type_num={cell_type_num}")
    ...
    print(f"  Model cluster_centers shape: {model.cluster_head.cluster_centers.shape}")
    ...
    print(f"  Val metrics: {ckpt.get('best_metrics', {})}")

    # Load state dict with strict=False (some cluster_head params may mismatch due to n_clusters)
    missing, unexpected = model.load_state_dict(ckpt["model_state_dict"], strict=False)
    print(f"  Loaded (missing={missing}, unexpected={unexpected})")

    print("Generating full embeddings...")
    ds = SimpleDataset(X_hvg, X_raw_v, hvg_idx, y_true_v)
    ...
    for k in [n_actual_clusters, n_actual_clusters-2, n_actual_clusters+2, n_actual_clusters-5, n_actual_clusters+5]:
        ...
        m = compute_metrics(y_true_v, pred)
    ...
    bm["checkpoint_epoch"] = ckpt.get("epoch", "?")
    bm["training_val_nmi"] = ckpt.get("best_metrics", {}).get("nmi", "?")
...
if __name__ == "__main__":
    ...
    run(a.data_path, a.result_dir, a.config_path, a.checkpoint_path, a.n_clusters, dev)
