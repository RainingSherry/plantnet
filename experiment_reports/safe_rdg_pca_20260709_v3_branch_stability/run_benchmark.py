...
BENCHMARK_PYTHON = Path("/data/luolie/conda/envs/scclubench-main/bin/python")
DEFAULT_PYTHON = BENCHMARK_PYTHON if BENCHMARK_PYTHON.exists() else Path(sys.executable)
...
def build_cmd(args: argparse.Namespace, ds: dict, seed: int, run_dir: Path) -> list[str]:
    return [
        ...
        "--stability_edge_drop_rate",
        str(args.stability_edge_drop_rate),
        "--stability_noise_scale",
        str(args.stability_noise_scale),
        "--stability_margin",
        str(args.stability_margin),
        "--stability_max_cells",
        str(args.stability_max_cells),
        "--assignment_stability_min",
        str(args.assignment_stability_min),
        "--graph_stability_min",
        str(args.graph_stability_min),
        "--cluster_entropy_min",
        str(args.cluster_entropy_min),
        "--largest_cluster_ratio_max",
        str(args.largest_cluster_ratio_max),
        "--hubness_max",
        str(args.hubness_max),
        "--spectral_failure_policy",
        args.spectral_failure_policy,
        "--save_embedding_h5",
        str(args.save_embedding_h5).lower(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--python_executable", type=Path, default=DEFAULT_PYTHON)
    ...
    parser.add_argument("--save_embedding_h5", type=lambda x: str(x).lower() in {"1", "true", "yes"}, default=False)
