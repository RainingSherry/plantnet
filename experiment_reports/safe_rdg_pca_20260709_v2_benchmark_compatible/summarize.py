...
def append_calibrated(df: pd.DataFrame, analysis_dir: Path) -> pd.DataFrame:
    ...
    for path in [analysis_dir / "calibrated_threshold_runs.csv", analysis_dir / "calibrated_logistic_runs.csv"]:
        if path.exists():
            ...
            parts.append(cal[["dataset", "seed", "variant", "ari", "nmi", "acc", "f1_macro", "q_total", "graph_enabled", "negative_control"]])
...
def success(by_variant: pd.DataFrame, by_dataset: pd.DataFrame, oracle_df: pd.DataFrame) -> dict:
    ...
    for variant in ["safe_rdg_heuristic", "safe_rdg_calibrated_threshold"]:
...
def main() -> int:
    ...
    report = {"q_correlations": q_correlations(df), "negative_controls": negative_control_report(df), "success": success(by_variant, by_dataset, oracle_df)}
