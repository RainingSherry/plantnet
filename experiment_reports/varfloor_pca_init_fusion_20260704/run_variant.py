...
ROOT = CURRENT_DIR.parents[1]  # experiment_reports/<dir>/ -> plantnet 仓库根
...
def parse_args():
    ...
    # 诊断变量: DEC 中心初始化来源 —— embedding(原版) vs pca_kmeans(用PCA-KMeans分配在隐空间求均值)
    parser.add_argument("--center_init", default="embedding", choices=["embedding", "pca_kmeans"])
...
def main() -> int:
    ...
    # === 诊断: 预计算 PCA(128) + PCA-KMeans(known-K) ===
    # PCA embedding 存下供事后融合(Step A); PCA-KMeans 分配供 DEC 初始化(Step B, center_init=pca_kmeans)。
    pca_dim = max(2, min(int(args.pca_dim), encoder_data.shape[0] - 1, encoder_data.shape[1] - 1))
    pca_emb = PCA(n_components=pca_dim, random_state=args.seed).fit_transform(
        encoder_data.astype(np.float64)).astype(np.float32)
    pca_km_pred = KMeans(n_clusters=n_clusters, n_init=20, random_state=args.seed).fit_predict(pca_emb)
    np.save(save_dir / "pca_embedding.npy", pca_emb)
    np.save(save_dir / "pca_kmeans_pred.npy", pca_km_pred.astype(np.int64))
    ...
    for epoch in range(1, max(1, args.epochs) + 1):
        if epoch > args.warmup_epochs and (
            ...
            if not centers_initialized:
                if args.center_init == "pca_kmeans":
                    # 用 PCA-KMeans 的划分, 在编码器隐空间(emb)对每簇求均值当 DEC 初始中心。
                    centers = np.stack([
                        emb[pca_km_pred == c].mean(axis=0) if (pca_km_pred == c).any()
                        else emb[np.random.RandomState(args.seed + c).randint(0, emb.shape[0])]
                        for c in range(n_clusters)]).astype(np.float32)
                else:  # embedding (原版): KMeans on encoder embedding
                    centers = KMeans(n_clusters=n_clusters, n_init=20,
                                     random_state=args.seed).fit(emb).cluster_centers_.astype(np.float32)
                model.initialize_centers(torch.as_tensor(centers, dtype=torch.float32, device=device))
                emb, q_full, _ = extract_all(model, full_loader, device)
