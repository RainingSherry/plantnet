# scVICAR：用于掩码单细胞表征学习的有界图邻域锚点恢复

> 本文档是英文投稿稿的中文对应阅读版。它用于核对论点、结果解释和措辞边界，不替代英文 LaTeX 正文。
...
**关键词：** 单细胞 RNA 测序；掩码自编码器；图表征学习；邻域风险；聚类；细胞类型注释。
...
1. 提出图邻域锚点恢复，以局部细胞几何构造有界输入破坏，并始终保持原始细胞为自监督目标。
2. 将拓扑自适应拆分为边亲和度与逐细胞混合系数，并分别进行组件消融和图污染实验。
3. 推导凸包、扰动、扩散和污染性质，并在受控图污染实验中检验相应量。
4. 建立分层评价体系：开发基准在 15 个数据集上比较 17 个完整方法配置，外部验证使用 6 个另行冻结的数据集，并进一步进行骨干匹配消融、图压力测试和防标签泄漏的下游任务。
...
主分析删除标准化标签为 `unknown`、`nan` 或 `unassigned` 的细胞，并删除少于 10 个细胞的类别。完整标签结果作为独立敏感性分析。`hrvatin_geo` 因标签语义无效而排除，也不与 `hrvatin` 作为两个独立数据集计数。每个确认性 H5AD 均由 SHA-256 唯一标识。
...
模型种子为 42、2024 和 3407。六个数据集、六个匹配变体和三个种子构成 108 次主要运行。主要终点是已知 $K$、`n_init=20` 的 KMeans ARI。NMI、assignment accuracy、macro-F1、silhouette、运行时间和峰值显存为次要指标。固定分辨率 1.0 的 Leiden 单独报告，不允许标签引导的分辨率搜索。
...
### 7.2 Marker-overlap annotation

每个预测簇被分配给 reference marker overlap 最大的类型，允许多个簇对应同一类型。并列时先比较共享 marker 的平均 log fold change，再按类型名排序。marker 少于 3 个或最大 overlap 为 0 的簇记为 `unassigned`。Hungarian mapping 仅作为 oracle upper bound，不作为实际注释方法。

### 7.3 低标签量冻结 embedding 探针

在 10% 和 30% 标签比例下，使用分层抽样训练 `StandardScaler + L2 multinomial logistic regression`。参数为 $C=1$、balanced class weights、最大迭代 2,000 次。Scaler 和分类器仅在训练子集上拟合，encoder 始终冻结。该任务衡量数据集内部的 transductive annotation。
...
1. **科学含义不准确**：例如某个机制解释与你原始设计意图不同。
2. **结论强弱不合适**：例如你希望某项结果在正文中更突出或更保守。
3. **术语不自然**：例如“图邻域锚点恢复”“逐细胞混合系数”是否符合你的中文表达习惯。

修改中文稿后，请逐段同步到英文 LaTeX。这样可以保持中英文证据边界一致。
