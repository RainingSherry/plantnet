...
def save(embedding_path, y, y_pred, epoch, embedding):
    ...
    with open(os.path.join(embedding_path, f'metrics_{epoch}.json'), 'w') as f:
        json.dump(metrics, f, indent=4)

    with h5py.File(os.path.join(embedding_path, 'embedding.h5'), 'w') as f:
        if isinstance(embedding, torch.Tensor):
            f.create_dataset('X', data=embedding.cpu().detach().numpy())
            np.save(os.path.join(embedding_path, f'embedding_{epoch}.npy'), embedding.cpu().detach().numpy())
        else:
